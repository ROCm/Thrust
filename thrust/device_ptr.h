...
/*! \file device_ptr.h
 *  \brief A pointer to a variable which resides in the "device" system's memory
 * space
 */
 ...
 */
    ...
    /*! \p device_ptr stores a pointer to an object allocated in device memory. This
 * type provides type safety when dispatching standard algorithms on ranges
 * resident in device memory.
 *
 *  \p device_ptr has pointer semantics: it may be dereferenced safely from the
 * host and may be manipulated with pointer arithmetic.
 *
 *  \p device_ptr can be created with the functions device_malloc, device_new,
 * or device_pointer_cast, or by explicitly calling its constructor with a raw
 * pointer.
 *
 *  The raw pointer encapsulated by a \p device_ptr may be obtained by either
 * its <tt>get</tt> method or the \p raw_pointer_cast free function.
 *
 *  \note \p device_ptr is not a smart pointer; it is the programmer's
 * responsibility to deallocate memory pointed to by \p device_ptr.
 *
 *  \see device_malloc
 *  \see device_new
 ...
 */
   ...
   */
        ...
        __host__ __device__ device_ptr(T* ptr)
            : super_t(ptr)
        {
        }

        /*! \p device_ptr's copy constructor allows copying from another device_ptr
   * with related type. \param other The \p device_ptr to copy from.
   */
        template <typename OtherT>
        ...
        /*! \p device_ptr's assignment operator allows assigning from another \p
   * device_ptr with related type. \param other The other \p device_ptr to copy
   * from. \return <tt>*this</tt>
   */
        template <typename OtherT>
 ...
 */
    ...
    /*! \p device_pointer_cast creates a device_ptr from a raw pointer which is
 * presumed to point to a location in device memory.
 *
 ...
 */
    ...
    /*! This version of \p device_pointer_cast creates a copy of a device_ptr from
 * another device_ptr. This version is included for symmetry with \p
 * raw_pointer_cast.
 *
