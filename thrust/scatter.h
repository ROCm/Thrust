 ...
 * RandomAccessIterator's \c value_type. \tparam InputIterator2 must be a model
 ...
 * iterator</a>.
 *
 *  \pre The iterator `result + i` shall not refer to any element referenced by
 * any iterator `j` in the range `[first,last)` for all iterators `i` in the
 * range `[map,map + (last - first))`.
 *
 *  \pre The iterator `result + i` shall not refer to any element referenced by
 * any iterator `j` in the range `[map,map + (last - first))` for all iterators
 * `i` in the range `[map,map + (last - first))`.
 *
 *  \pre The expression `result[*i]` shall be valid for all iterators in the
 * range `[map,map + (last - first))`.
 *
 *  The following code snippet demonstrates how to use \p scatter to
 *  reorder a range using the \p thrust::device execution policy for
 * parallelization:
 *
 *  \code
 ...
 * of <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input
 * Iterator</a> and \c InputIterator2's \c value_type must be convertible to \c
 * RandomAccessIterator's \c difference_type. \tparam InputIterator3 must be a
 * model of <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input
 * Iterator</a> and \c InputIterator3's \c value_type must be convertible to \c
 ...
 *
 *  \pre The iterator `result + i` shall not refer to any element referenced by
 * any iterator `j` in the range `[stencil,stencil + (last - first))` for all
 * iterators `i` in the range `[map,map + (last - first))`.
 *
 *  \pre The expression `result[*i]` shall be valid for all iterators `i` in the
 * range `[map,map + (last - first))` for which the following condition holds:
 * `*(stencil + i) != false`.
 *
 *  \code
 *  #include <thrust/scatter.h>
 ...
 */
    ...
    /*! \p scatter_if conditionally copies elements from a source range into an
 *  output array according to a map. For each iterator \c i in the
 *  range <tt>[first, last)</tt> such that <tt>pred(*(stencil + (i -
 * first)))</tt> is \c true, the value \c *i is assigned to <tt>output[*(map +
 * (i - first))]</tt>. The output iterator must permit random access. If the
 * same index appears more than once in the range <tt>[map, map + (last -
 * first))</tt> the result is undefined.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 *  \param exec The execution policy to use for parallelization.
