 ...
 */
    ...
    /*! \p replace replaces every element in the range [first, last) equal to \p
 * old_value with \p new_value. That is: for every iterator \c i, if <tt>*i ==
 * old_value</tt> then it performs the <tt>assignment *i = new_value</tt>.
 *
 ...
 * href="http://www.sgi.com/tech/stl/Assignable.html>Assignable.html">Assignable</a>,
 *          \p T is a model of <a
 ...
 *  a value of interest in a \c device_vector with another using the \p
 ...
 */
    ...
    /*! \p replace_if replaces every element in the range <tt>[first, last)</tt> for
 * which \p pred returns \c true with \p new_value. That is: for every iterator
 * \c i, if <tt>pred(*i)</tt> is \c true then it performs the assignment <tt>*i
 * = new_value</tt>.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 ...
 *  \param first The beginning of the sequence of interest.
 *  \param last The end of the sequence of interest.
 *  \param pred The predicate to test on every value of the range
 * <tt>[first,last)</tt>. \param new_value The new value to replace elements
 * which <tt>pred(*i)</tt> evaluates to \c true.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *  \tparam ForwardIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/ForwardIterator.html">Forward Iterator</a>,
 ...
 * href="http://www.sgi.com/tech/stl/Predicate.html">Predicate</a>. \tparam T is
 * a model of <a
 ...
 *  a \c device_vector's negative elements with \c 0 using the \p thrust::device
 ...
 */
    ...
    /*! \p replace_if replaces every element in the range <tt>[first, last)</tt> for
 * which <tt>pred(*s)</tt> returns \c true with \p new_value. That is: for every
 * iterator \c i in the range <tt>[first, last)</tt>, and \c s in the range
 * <tt>[stencil, stencil + (last - first))</tt>, if <tt>pred(*s)</tt> is \c true
 * then it performs the assignment <tt>*i = new_value</tt>.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 *  \param exec The execution policy to use for parallelization.
 ...
 *
 *  The following code snippet demonstrates how to use \p replace_if to replace
 *  a \c device_vector's element with \c 0 when its corresponding stencil
 * element is less than zero using the \p thrust::device execution policy for
 * parallelization:
 *
 *  \code
 *  #include <thrust/replace.h>
 ...
 */
    ...
    /*! \p replace_copy copies elements from the range <tt>[first, last)</tt> to the
 * range <tt>[result, result + (last-first))</tt>, except that any element equal
 * to \p old_value is not copied; \p new_value is copied instead.
 *
 *  More precisely, for every integer \c n such that <tt>0 <= n <
 * last-first</tt>, \p replace_copy performs the assignment <tt>*(result+n) =
 * new_value</tt> if <tt>*(first+n) == old_value</tt>, and <tt>*(result+n) =
 * *(first+n)</tt> otherwise.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 *  \param exec The execution policy to use for parallelization.
 ...
 *  \param old_value The value to replace.
 *  \param new_value The replacement value for which <tt>*i == old_value</tt>
 * evaluates to \c true. \return <tt>result + (last-first)</tt>
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *  \tparam InputIterator is a model of <a
 ...
 *  \tparam T is a model of <a
 * href="http://www.sgi.com/tech/stl/Assignable.html">Assignable</a>, \p T is a
 * model of <a
 * href="http://www.sgi.com/tech/stl/EqualityComparable.html">Equality
 * Comparable</a>, \p T may be compared for equality with \p InputIterator's \c
 * value_type, and \p T is convertible to \p OutputIterator's \c value_type.
 *
 *  \pre \p first may equal \p result, but the ranges <tt>[first, last)</tt> and
 ...
 */
    ...
    /*! \p replace_copy_if copies elements from the range <tt>[first, last)</tt> to
 the range
 *  <tt>[result, result + (last-first))</tt>, except that any element for which
 \p pred
 *  is \c true is not copied; \p new_value is copied instead.
 *
 *  More precisely, for every integer \c n such that 0 <= n < last-first,
 *  \p replace_copy_if performs the assignment <tt>*(result+n) = new_value</tt>
 if
 ...
 *  \param pred The predicate to test on every value of the range
 <tt>[first,last)</tt>.
 *  \param new_value The replacement value to assign <tt>pred(*i)</tt> evaluates
 to \c true.
 ...
 *  is_less_than_zero pred;
 *
 *  thrust::replace_copy_if(thrust::device, A.begin(), A.end(), B.begin(), pred,
 0);
 *
 *  // B contains [1, 0, 2, 0]
 *  \endcode
 ...
 */
    ...
    /*! This version of \p replace_copy_if copies elements from the range
 * <tt>[first, last)</tt> to the range <tt>[result, result + (last-first))</tt>,
 * except that any element whose corresponding stencil element causes \p pred to
 * be \c true is not copied; \p new_value is copied instead.
 *
 *  More precisely, for every integer \c n such that <tt>0 <= n <
 * last-first</tt>, \p replace_copy_if performs the assignment <tt>*(result+n) =
 * new_value</tt> if <tt>pred(*(stencil+n))</tt>, and <tt>*(result+n) =
 * *(first+n)</tt> otherwise.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 ...
 * stencil + (last - first))</tt>. \param new_value The replacement value to
 * assign when <tt>pred(*s)</tt> evaluates to \c true. \return <tt>result +
 * (last-first)</tt>
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 ...
 * a model of <a
 * href="http://www.sgi.com/tech/stl/Assignable.html">Assignable</a>, and \p T
 * is convertible to \p OutputIterator's \c value_type.
 *
 *  \pre \p first may equal \p result, but the ranges <tt>[first, last)</tt> and
 * <tt>[result, result + (last - first))</tt> shall not overlap otherwise. \pre
 * \p stencil may equal \p result, but the ranges <tt>[stencil, stencil + (last
 ...
 *  is_less_than_zero pred;
 *
 *  thrust::replace_if(thrust::device, A.begin(), A.end(), S.begin(), B.begin(),
 * pred, 0);
 *
