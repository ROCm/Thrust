...
namespace thrust
{
    ...
    // for reasonable reference-like semantics, derived types must reimplement the
    // following:
    ...
    private:
        ...
        // hint for is_wrapped_reference lets it know that this type (or a derived
        // type) is a wrapped reference
    ...
    public:
        ...
        __host__ __device__ derived_type& operator=(const reference& other);

        // XXX this may need an enable_if
        template <typename OtherElement, typename OtherPointer, typename OtherDerived>
        __host__ __device__ derived_type&
                            operator=(const reference<OtherElement, OtherPointer, OtherDerived>& other);

        __host__ __device__ derived_type& operator=(const value_type& x);

        __host__ __device__ pointer operator&() const;

        __host__ __device__ operator value_type() const;

        __host__ __device__ void swap(derived_type& other);
    ...
    private:
        ...
        __host__ __device__ inline value_type strip_const_get_value(const System& system) const;

        template <typename OtherPointer>
        __host__ __device__ inline void assign_from(OtherPointer src);

        // XXX this helper exists only to avoid warnings about null references from
        // the other assign_from
        ...
        __host__ __device__ inline void strip_const_assign_value(const System& system,
                                                                 OtherPointer  src);

        // XXX this helper exists only to avoid warnings about null references from
        // the other swap
        template <typename System>
        inline __host__ __device__ void swap(System* system, derived_type& other);

        // XXX this helper exists only to avoid warnings about null references from
        // operator value_type ()
        template <typename System>
        inline __host__ __device__ value_type convert_to_value_type(System* system) const;
    }; // end reference
