...
namespace thrust
{
    namespace detail
    {
        ...
        class temporary_allocator
            : public thrust::detail::tagged_allocator<T, System, thrust::pointer<T, System>>
        {
        private:
        ...
        public:
            ...
            inline __host__ __device__ temporary_allocator(const temporary_allocator& other)
                : super_t()
            ...
            inline __host__
                __device__ explicit temporary_allocator(thrust::execution_policy<System>& system)
                : super_t()
                , m_system(thrust::detail::derived_cast(system))
            {
            }

            __host__ __device__ pointer allocate(size_type cnt);

            __host__ __device__ void deallocate(pointer p, size_type n);

            __host__ __device__ inline System& system()
            {
                return m_system;
            } // end system()
