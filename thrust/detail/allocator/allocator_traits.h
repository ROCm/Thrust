...
namespace thrust
{
    namespace detail
    {
        ...
        namespace allocator_traits_detail
        {
            ...
            __THRUST_DEFINE_HAS_NESTED_TYPE(has_propagate_on_container_copy_assignment,
                                            propagate_on_container_copy_assignment)
            __THRUST_DEFINE_HAS_NESTED_TYPE(has_propagate_on_container_move_assignment,
                                            propagate_on_container_move_assignment)
            __THRUST_DEFINE_HAS_NESTED_TYPE(has_propagate_on_container_swap,
        ...
        public:
            ...
            typedef typename eval_if<
                allocator_traits_detail::has_propagate_on_container_copy_assignment<
                    allocator_type>::value,
                allocator_traits_detail::nested_propagate_on_container_copy_assignment<
                    allocator_type>,
                identity_<false_type>>::type propagate_on_container_copy_assignment;

            typedef typename eval_if<
                allocator_traits_detail::has_propagate_on_container_move_assignment<
                    allocator_type>::value,
                allocator_traits_detail::nested_propagate_on_container_move_assignment<
                    allocator_type>,
            ...
            inline __host__ __device__ static pointer allocate(allocator_type& a, size_type n);

            inline __host__ __device__ static pointer
                            allocate(allocator_type& a, size_type n, const_void_pointer hint);

            inline __host__ __device__ static void
            ...
            inline __host__ __device__ static size_type max_size(const allocator_type& a);
        }; // end allocator_traits

        // we consider a type an allocator if T::value_type exists
        // it doesn't make much sense (containers, which are not allocators, will
        // fulfill this requirement), but allocator_traits is specified to work for any
        // type with that nested typedef
        template <typename T>
        ...
        struct allocator_system
        {
            ...
            typedef typename eval_if<allocator_traits_detail::has_member_system<
                                         Alloc>::value, // if Alloc.system() exists
                                     add_reference<type>, // then get() needs to return a
                                     // reference
                                     identity_<type> // else get() needs to return a value
                                     >::type get_result_type;

            __host__ __device__ inline static get_result_type get(Alloc& a);
        };
