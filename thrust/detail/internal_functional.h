...
namespace thrust
{
    namespace detail
    {
        ...
        struct unary_negate
        {
            ...
            __host__ __device__ explicit unary_negate(const Predicate& pred)
                : pred(pred)
            {
            }

            // workaround
            __host__ __device__ explicit unary_negate() {}
        ...
        // binary_negate does not need to know first_argument_type or
        ...
        struct binary_negate
        {
            ...
            __host__ __device__ explicit binary_negate(const Predicate& pred)
                : pred(pred)
            {
            }
            // workaround
            __host__ __device__ explicit binary_negate() {}

            template <typename T1, typename T2>
            __host__ __device__ bool operator()(const T1& x, const T2& y)
            {
        ...
        template <typename Predicate>
        __host__ __device__ thrust::detail::unary_negate<Predicate> not1(const Predicate& pred)
        {
            return thrust::detail::unary_negate<Predicate>(pred);
        }

        template <typename Predicate>
        __host__ __device__ thrust::detail::binary_negate<Predicate> not2(const Predicate& pred)
        {
        ...
        struct predicate_to_integral
        {
            ...
            __host__ __device__ explicit predicate_to_integral(const Predicate& pred)
                : pred(pred)
            {
            }

            // workaround
            __host__ __device__ explicit predicate_to_integral() {}
            template <typename T>
            __host__ __device__ bool operator()(const T& x)
            {
        ...
        // note that detail::equal_to does not force conversion from T2 -> T1 as
        // equal_to does
        ...
        struct equal_to
        {
            ...
            __host__ __device__ bool operator()(const T1& lhs, const T2& rhs) const
            {
                return lhs == rhs;
            }
        };

        // note that equal_to_value does not force conversion from T2 -> T1 as equal_to
        // does
        template <typename T2>
        struct equal_to_value
        {
            ...
            __host__ __device__ equal_to_value(const T2& rhs)
                : rhs(rhs)
            {
            }

            template <typename T1>
            __host__ __device__ bool operator()(const T1& lhs) const
            {
                return lhs == rhs;
            }
        };
        ...
        struct tuple_binary_predicate
        {
            ...
            __host__ __device__ tuple_binary_predicate(const Predicate& p)
                : pred(p)
            {
            }

            // workaround
            __host__ __device__ tuple_binary_predicate() {}
        ...
        struct tuple_not_binary_predicate
        {
            ...
            __host__ __device__ tuple_not_binary_predicate(const Predicate& p)
                : pred(p)
            {
            }
        ...
        struct host_generate_functor
        {
            ...
            __host__ __device__ host_generate_functor(Generator g)
                : gen(g)
            {
            }
            ...
            template <typename T>
            __host__ __device__ void operator()(const T& x)
            {
        ...
        struct device_generate_functor
        {
            ...
            __host__ __device__ device_generate_functor(Generator g)
                : gen(g)
            {
            }
        ...
        struct zipped_binary_op
        {
            ...
            __host__ __device__ zipped_binary_op(BinaryFunction binary_op)
                : m_binary_op(binary_op)
            {
            }

            // workaround
            __host__ __device__ zipped_binary_op() {}

            template <typename Tuple>
            __host__ __device__ inline result_type operator()(Tuple t)
            {
        ...
        struct is_tuple_of_iterator_references<
            thrust::detail::tuple_of_iterator_references<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10>>
            : thrust::detail::true_type
        {
        };

        // use this enable_if to avoid assigning to temporaries in the transform
        // functors below
        ...
        struct enable_if_non_const_reference_or_tuple_of_iterator_references
            : thrust::detail::enable_if<is_non_const_reference<T>::value
                                        || is_tuple_of_iterator_references<T>::value>
        {
        };
        ...
        struct unary_transform_functor
        {
            ...
            __host__ __device__ unary_transform_functor(UnaryFunction f)
                : f(f)
            {
            }

            // Stub start`
            __host__ __device__ unary_transform_functor()
                : f()
            {
            }
            // end
            __thrust_exec_check_disable__ template <typename Tuple>
        ...
        struct binary_transform_functor
        {
            ...
            __host__ __device__ binary_transform_functor(BinaryFunction f)
                : f(f)
            {
            }

            // workaround for compilation error
            __host__ __device__ binary_transform_functor()
                : f()
            {
            }
        ...
        struct unary_transform_if_functor
        {
            ...
            __host__ __device__ unary_transform_if_functor(UnaryFunction unary_op, Predicate pred)
                : unary_op(unary_op)
                , pred(pred)
            {
            }
            // Workaround for compilation error
            __host__ __device__ unary_transform_if_functor() {}
        ...
        struct unary_transform_if_with_stencil_functor
        {
            ...
            __host__ __device__ unary_transform_if_with_stencil_functor(UnaryFunction unary_op,
                                                                        Predicate     pred)
                : unary_op(unary_op)
                , pred(pred)
            {
            ...
            // workaround
            __host__ __device__ unary_transform_if_with_stencil_functor() {}
        ...
        struct binary_transform_if_functor
        {
            ...
            __host__ __device__ binary_transform_if_functor(BinaryFunction binary_op,
                                                            Predicate      pred)
                : binary_op(binary_op)
                , pred(pred)
            {
            ...
            // workaround
            __host__ __device__ binary_transform_if_functor() {}
            __thrust_exec_check_disable__ template <typename Tuple>
            inline __host__ __device__
                typename enable_if_non_const_reference_or_tuple_of_iterator_references<
        ...
        {
            __host__ void operator()(T& x) const
            {
                x.~T();
            } // end operator()()
        ...
        struct fill_functor
        {
            ...
            __host__ __device__ fill_functor(const T& _exemplar)
                : exemplar(_exemplar)
            {
            }

            __host__ __device__ T operator()(void) const
            {
                return exemplar;
            }
        };
        ...
        struct uninitialized_fill_functor
        {
            ...
            __host__ __device__ uninitialized_fill_functor(T x)
                : exemplar(x)
            {
            }
        ...
        struct compare_first_less_second
        {
            ...
            template <typename T1, typename T2>
            __host__ __device__ bool operator()(T1 lhs, T2 rhs)
            {
                return comp(thrust::get<0>(lhs), thrust::get<0>(rhs))
                       || (!comp(thrust::get<0>(rhs), thrust::get<0>(lhs))
        ...
        struct compare_first
        {
            ...
            __host__ __device__ compare_first() {}
            __host__ __device__ compare_first(Compare comp)
                : comp(comp)
            {
            }

            template <typename Tuple1, typename Tuple2>
            __host__ __device__ bool operator()(const Tuple1& x, const Tuple2& y)
            {
                return comp(thrust::raw_reference_cast(thrust::get<0>(x)),
                            thrust::raw_reference_cast(thrust::get<0>(y)));
            }
        }; // end compare_first
