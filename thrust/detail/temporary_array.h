...
namespace thrust
{
    namespace detail
    {
        ...
        class temporary_array
            : public contiguous_storage<T, no_throw_allocator<temporary_allocator<T, System>>>
        {
        private:
            typedef contiguous_storage<T, no_throw_allocator<temporary_allocator<T, System>>>
                super_t;
        ...
        public:
            ...
            __host__ __device__ temporary_array(thrust::execution_policy<System>& system);

            __host__ __device__ temporary_array(thrust::execution_policy<System>& system,
                                                size_type                         n);

            // provide a kill-switch to explicitly avoid initialization
            __host__ __device__ temporary_array(int                               uninit,
            ...
            __host__ __device__ ~temporary_array();
        }; // end temporary_array
        ...
        // note that the resulting iterator is explicitly tagged with ToSystem either
        // way
        template <typename Iterator, typename FromSystem, typename ToSystem>
        struct move_to_system_base
            : public eval_if<
                  is_convertible<FromSystem, ToSystem>::value,
                  identity_<tagged_iterator_range<Iterator, ToSystem>>,
                  identity_<
                      temporary_array<typename thrust::iterator_value<Iterator>::type, ToSystem>>>
        {
        };
