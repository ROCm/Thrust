...
namespace thrust
{
    ...
    namespace detail
    {
        ...
        // this metafunction computes the type of iterator_adaptor thrust::pointer
        // should inherit from
        template <typename Element, typename Tag, typename Reference, typename Derived>
        struct pointer_base
        {
            ...
            typedef typename thrust::detail::eval_if<
                thrust::detail::is_void<
                    typename thrust::detail::remove_const<Element>::type>::value,
            ...
            typedef typename thrust::detail::eval_if<
                ...
                thrust::detail::eval_if<thrust::detail::is_same<Reference, use_default>::value,
                                        ...
                                        thrust::detail::identity_<Reference>>>::type reference_arg;

            typedef thrust::iterator_adaptor<
                derived_type, // pass along the type of our Derived class to
                // iterator_adaptor
                Element*, // we adapt a raw pointer
                value_type, // the value type
                Tag, // system tag
                thrust::random_access_traversal_tag, // pointers have random access
                // traversal
    ...
    // for reasonable pointer-like semantics, derived types should reimplement the
    // following:
    ...
    private:
        typedef
            typename thrust::detail::pointer_base<Element, Tag, Reference, Derived>::type super_t;

        typedef
            typename thrust::detail::pointer_base<Element, Tag, Reference, Derived>::derived_type
                derived_type;
    ...
    public:
        ...
        // XXX consider making the pointer implementation a template parameter which
        // defaults to Element *
        template <typename OtherElement>
        __host__ __device__ explicit pointer(OtherElement* ptr);

        // added to check linker errors
        __host__ __device__ explicit pointer(Element* ptr);

        // OtherPointer's element_type shall be convertible to Element
        // OtherPointer's system shall be convertible to Tag
        template <typename OtherPointer>
        __host__ __device__ pointer(const OtherPointer& other,
        ...
        __host__ __device__ typename thrust::detail::
            enable_if_pointer_is_convertible<OtherPointer, pointer, derived_type&>::type
            operator=(const OtherPointer& other);

        // observers

        __host__ __device__ Element* get() const;
    }; // end pointer
