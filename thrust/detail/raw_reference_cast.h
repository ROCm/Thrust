...
// this header defines raw_reference_cast, which has a few overloads towards the
// bottom of the file raw_reference_cast depends on metafunctions such as
// is_unwrappable and raw_reference we need to be sure that these metafunctions
// are completely defined (including specializations) before they are
...
namespace thrust
{
    namespace detail
    {
        ...
        template <typename T0,
                  ...
                  typename T8,
                  typename T9>
        struct is_unwrappable<thrust::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>>
            : or_<is_unwrappable<T0>,
                  is_unwrappable<T1>,
                  is_unwrappable<T2>,
                  is_unwrappable<T3>,
                  is_unwrappable<T4>,
        ...
        // specialize is_unwrappable
        // a tuple_of_iterator_references is_unwrappable if any of its elements
        // is_unwrappable
        template <typename T0,
                  typename T1,
                  typename T2,
        ...
        namespace raw_reference_detail
        {
            ...
            template <typename T>
            struct raw_reference_impl<T,
                                      typename thrust::detail::enable_if<is_wrapped_reference<
                                          typename remove_cv<T>::type>::value>::type>
            {
                typedef typename add_reference<
                    typename pointer_element<typename T::pointer>::type>::type type;
            };
        ...
        namespace raw_reference_detail
        {
            ...
            //  4. reference<T>                     -> T&
            //  5. tuple_of_iterator_references<T>  ->
            //  tuple_of_iterator_references<raw_reference_tuple_helper<T>::type>

            // wrapped references are unwrapped using raw_reference, otherwise, return T
            template <typename T>
            struct raw_reference_tuple_helper
                : eval_if<is_unwrappable<typename remove_cv<T>::type>::value,
                          raw_reference<T>,
                          identity_<T>>
            {
            };
            ...
            template <typename T0,
                      ...
                      typename T8,
                      typename T9>
            struct raw_reference_tuple_helper<thrust::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>>
            {
                typedef thrust::tuple<typename raw_reference_tuple_helper<T0>::type,
            ...
            struct raw_reference_tuple_helper<
                thrust::detail::
                    tuple_of_iterator_references<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>>
            {
        ...
        //   then the raw_reference of tuple_type is a tuple of its members'
        //   raw_references else the raw_reference of tuple_type is tuple_type &
        template <typename T0,
                  typename T1,
                  typename T2,
                  typename T3,
                  ...
                  typename T8,
                  typename T9>
        struct raw_reference<thrust::tuple<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>>
        {
        private:
        ...
        public:
            typedef typename eval_if<is_unwrappable<tuple_type>::value,
                                     ...
                                     add_reference<tuple_type>>::type type;
        };
        ...
        {
        private:
            typedef detail::tuple_of_iterator_references<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>
                tuple_type;

        public:
            typedef
                typename raw_reference_detail::raw_reference_tuple_helper<tuple_type>::type type;

            // XXX figure out why is_unwrappable seems to be broken for
    ...
    // provide declarations of raw_reference_cast's overloads for
    // raw_reference_caster below
    template <typename T>
    inline __host__ __device__ typename detail::raw_reference<T>::type raw_reference_cast(T& ref);

    template <typename T>
    inline __host__ __device__ typename detail::raw_reference<const T>::type
    ...
    __host__ __device__ typename detail::enable_if_unwrappable<
        ...
        typename detail::raw_reference<
            ...
            type>::type
        raw_reference_cast(
            thrust::detail::tuple_of_iterator_references<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> t);
    ...
    namespace detail
    {
        ...
        {
            template <typename T>
            __host__ __device__ typename detail::raw_reference<T>::type operator()(T& ref)
            {
            ...
            __host__ __device__ typename detail::raw_reference<
                thrust::detail::
                    tuple_of_iterator_references<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9>>::type
                operator()(
                    thrust::detail::
                        tuple_of_iterator_references<T0, T1, T2, T3, T4, T5, T6, T7, T8, T9> t,
