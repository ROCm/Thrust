...
namespace thrust
{
    ...
    namespace random
    {
        ...
        /*! \addtogroup random_number_engine_adaptors Random Number Engine Adaptor Class
 * Templates \ingroup random
 *  \{
 */

        /*! \class discard_block_engine
 *  \brief A \p discard_block_engine adapts an existing base random number
 * engine and produces random values by discarding some of the values returned
 * by its base engine. Each cycle of the compound engine begins by returning \c
 * r values successively produced by the base engine and ends by discarding
 * <tt>p-r</tt> such values. The engine's state is the state of its base engine
 * followed by the number of calls to <tt>operator()</tt> that have occurred
 ...
 *  \tparam r The number of values to return of the base engine. Because
 * <tt>p-r</tt> will be discarded, <tt>r <= p</tt>.
 *
 *  The following code snippet shows an example of using a \p
 * discard_block_engine instance:
 *
 *  \code
 ...
 *  int main(void)
 *  {
 *    // create a discard_block_engine from minstd_rand, with a cycle length of
 * 13
 ...
 */
   ...
   *
   *  \param urng A \p base_type to use to initialize this \p
   * discard_block_engine's adapted base engine.
   */
            __host__ __device__ explicit discard_block_engine(const base_type& urng);

            /*! This constructor initializes a new \p discard_block_engine with a given
   * seed.
   *
   *  \param s The seed used to intialize this \p discard_block_engine's adapted
   * base engine.
   */
            __host__ __device__ explicit discard_block_engine(result_type s);

            /*! This method initializes the state of this \p discard_block_engine's
   * adapted base engine by using its \p default_seed value.
   */
            __host__ __device__ void seed(void);

            /*! This method initializes the state of this \p discard_block_engine's
   * adapted base engine by using the given seed.
   *
   *  \param s The seed with which to intialize this \p discard_block_engine's
   * adapted base engine.
   */
   ...
   * discard_block_engine's state. \return A new random number.
   */
            __host__ __device__ result_type operator()(void);

            /*! This member function advances this \p discard_block_engine's state a given
   ...
   * discard_block_engine's adapted base engine.
   *
   *  \return A const reference to the base engine this \p discard_block_engine
   * adapts.
   */
   ...
   */
        private:
            ...
            __host__ __device__ bool equal(const discard_block_engine& rhs) const;
 ...
 */
        template <typename Engine, size_t p, size_t r>
        __host__ __device__ bool operator==(const discard_block_engine<Engine, p, r>& lhs,
