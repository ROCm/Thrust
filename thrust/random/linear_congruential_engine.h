...
namespace thrust
{
    ...
    namespace random
    {
        ...
        /*! \addtogroup random_number_engine_templates Random Number Engine Class
 * Templates \ingroup random
 *  \{
 */

        /*! \class linear_congruential_engine
 *  \brief A \p linear_congruential_engine random number engine produces
 * unsigned integer random numbers using a linear congruential random number
 * generation algorithm.
 *
 *         The generation algorithm has the form <tt>x_i = (a * x_{i-1} + c) mod
 * m</tt>.
 *
 ...
 *
 *  \note Inexperienced users should not use this class template directly.
 * Instead, use \p minstd_rand or \p minstd_rand0.
 *
 *  The following code snippet shows examples of use of a \p
 * linear_congruential_engine instance:
 *
 *  \code
 ...
 *  int main(void)
 *  {
 *    // create a minstd_rand object, which is an instance of
 * linear_congruential_engine thrust::minstd_rand rng1;
 *
 ...
 */
        ...
        public:
            ...
            /*! \typedef result_type
   *  \brief The type of the unsigned integer produced by this \p
   ...
   */
            ...
            /*! The smallest value this \p linear_congruential_engine may potentially
   * produce.
   */
            static const result_type min = c == 0u ? 1u : 0u;

            /*! The largest value this \p linear_congruential_engine may potentially
   * produce.
   */
   ...
   *
   *  \param s The seed used to intialize this \p linear_congruential_engine's
   * state.
   */
            __host__ __device__ explicit linear_congruential_engine(result_type s = default_seed);

            /*! This method initializes this \p linear_congruential_engine's state, and
   * optionally accepts a seed value.
   *
   *  \param s The seed used to initializes this \p linear_congruential_engine's
   * state.
   */
            ...
            /*! This member function produces a new random value and updates this \p
   * linear_congruential_engine's state. \return A new random number.
   */
            __host__ __device__ result_type operator()(void);

            /*! This member function advances this \p linear_congruential_engine's state a
   * given number of times and discards the results.
   *
   *  \param z The number of random values to discard.
   *  \note This function is provided because an implementation may be able to
   * accelerate it.
   */
   ...
   */
        private:
            ...
            __host__ __device__ bool equal(const linear_congruential_engine& rhs) const;
 ...
 */
        ...
        /*! This function streams a linear_congruential_engine to a \p
 * std::basic_ostream. \param os The \p basic_ostream to stream out to. \param e
 * The \p linear_congruential_engine to stream out. \return \p os
 */
        ...
        /*! This function streams a linear_congruential_engine in from a
 * std::basic_istream. \param is The \p basic_istream to stream from. \param e
 * The \p linear_congruential_engine to stream in. \return \p is
 */
        template <typename UIntType_,
                  UIntType_ a_,
                  UIntType_ c_,
 ...
 * version of the Minimal Standard random number generation algorithm. \note The
 * 10000th consecutive invocation of a default-constructed object of type \p
 * minstd_rand0 shall produce the value \c 1043618065 .
 */
        typedef linear_congruential_engine<thrust::detail::uint32_t, 16807, 0, 2147483647>
            minstd_rand0;
 ...
 * version of the Minimal Standard random number generation algorithm. \note The
 * 10000th consecutive invocation of a default-constructed object of type \p
 * minstd_rand shall produce the value \c 399268537 .
 */
        typedef linear_congruential_engine<thrust::detail::uint32_t, 48271, 0, 2147483647>
            minstd_rand;
