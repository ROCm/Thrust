 ...
 *         unsigned integer random values using a linear feedback shift random
 * number generation algorithm.
 *
 *  \tparam UIntType The type of unsigned integer to produce.
 *  \tparam w The word size of the produced values (<tt>w <=
 * sizeof(UIntType)</tt>). \tparam k The k parameter of Tausworthe's 1965
 * algorithm. \tparam q The q exponent of Tausworthe's 1965 algorithm. \tparam s
 * The step size of Tausworthe's 1965 algorithm.
 *
 *  \note linear_feedback_shift_engine is based on the Boost Template Library's
 ...
 */
   ...
   */
        private:
            static const result_type wordmask
                = detail::linear_feedback_shift_engine_wordmask<result_type, w>::value;
            /*! \endcond
   */

        public:
            /*! The smallest value this \p linear_feedback_shift_engine may potentially
   * produce.
   */
            static const result_type min = 0;

            /*! The largest value this \p linear_feedback_shift_engine may potentially
   * produce.
   */
   ...
   * linear_feedback_shift_engine's state.
   */
            __host__ __device__ explicit linear_feedback_shift_engine(result_type value
                                                                      = default_seed);

            /*! This method initializes this \p linear_feedback_shift_engine's state, and
   ...
   * linear_feedback_shift_engine's state. \return A new random number.
   */
            __host__ __device__ result_type operator()(void);

            /*! This member function advances this \p linear_feedback_shift_engine's state
   * a given number of times and discards the results.
   *
   ...
   */
        private:
            ...
            __host__ __device__ bool equal(const linear_feedback_shift_engine& rhs) const;
 ...
 */
        ...
        /*! This function streams a linear_feedback_shift_engine to a \p
 * std::basic_ostream. \param os The \p basic_ostream to stream out to. \param e
 * The \p linear_feedback_shift_engine to stream out. \return \p os
 */
        ...
        /*! This function streams a linear_feedback_shift_engine in from a
 * std::basic_istream. \param is The \p basic_istream to stream from. \param e
 * The \p linear_feedback_shift_engine to stream in. \return \p is
 */
        template <typename UIntType_,
                  size_t w_,
                  size_t k_,
