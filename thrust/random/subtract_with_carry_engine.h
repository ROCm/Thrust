 ...
 *  \brief A \p subtract_with_carry_engine random number engine produces
 * unsigned integer random numbers using the subtract with carry algorithm of
 * Marsaglia & Zaman.
 *
 *         The generation algorithm is performed as follows:
 *         -# Let <tt>Y = X_{i-s}- X_{i-r} - c</tt>.
 *         -# Set <tt>X_i</tt> to <tt>y = T mod m</tt>. Set \c c to \c 1 if
 * <tt>Y < 0</tt>, otherwise set \c c to \c 0.
 *
 *         This algorithm corresponds to a modular linear function of the form
 *
 *         <tt>TA(x_i) = (a * x_i) mod b</tt>, where \c b is of the form <tt>m^r
 * - m^s + 1</tt> and <tt>a = b - (b-1)/m</tt>.
 *
 *  \tparam UIntType The type of unsigned integer to produce.
 *  \tparam w The word size of the produced values (<tt> w <=
 * sizeof(UIntType)</tt>). \tparam s The short lag of the generation algorithm.
 *  \tparam r The long lag of the generation algorithm.
 *
 *  \note Inexperienced users should not use this class template directly.
 * Instead, use \p ranlux24_base or \p ranlux48_base, which are instances of \p
 ...
 */
   ...
   */
            ...
            /*! The smallest value this \p subtract_with_carry_engine may potentially
   * produce.
   */
            static const result_type min = 0;

            /*! The largest value this \p subtract_with_carry_engine may potentially
   * produce.
   */
   ...
   *
   *  \param value The seed used to intialize this \p
   * subtract_with_carry_engine's state.
   */
            __host__ __device__ explicit subtract_with_carry_engine(result_type value
                                                                    = default_seed);

            /*! This method initializes this \p subtract_with_carry_engine's state, and
   * optionally accepts a seed value.
   *
   *  \param value The seed used to initializes this \p
   * subtract_with_carry_engine's state.
   */
   ...
   * subtract_with_carry_engine's state. \return A new random number.
   */
            __host__ __device__ result_type operator()(void);

            /*! This member function advances this \p subtract_with_carry_engine's state a
   ...
   */
        private:
            ...
            __host__ __device__ bool equal(const subtract_with_carry_engine& rhs) const;
 ...
 */
        ...
        /*! This function streams a subtract_with_carry_engine to a \p
 * std::basic_ostream. \param os The \p basic_ostream to stream out to. \param e
 * The \p subtract_with_carry_engine to stream out. \return \p os
 */
        ...
        /*! This function streams a subtract_with_carry_engine in from a
 * std::basic_istream. \param is The \p basic_istream to stream from. \param e
 * The \p subtract_with_carry_engine to stream in. \return \p is
 */
        template <typename UIntType_,
                  size_t w_,
                  size_t s_,
 ...
 * consecutive invocation of a default-constructed object of type \p
 * ranlux24_base shall produce the value \c 7937952 .
 */
 ...
 *  \brief A random number engine with predefined parameters which implements
 * the base engine of the \p ranlux48 random number engine. \note The 10000th
 * consecutive invocation of a default-constructed object of type \p
 * ranlux48_base shall produce the value \c 192113843633948 .
 */
