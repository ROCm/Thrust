 ...
 *  \brief An \p xor_combine_engine adapts two existing base random number
 * engines and produces random values by combining the values produced by each.
 *
 ...
 *  \tparam s2 The second of the second shift to use in the generation
 * algorithm. Defaults to \c 0.
 *
 *  The following code snippet shows an example of using an \p
 * xor_combine_engine instance:
 *
 *  \code
 ...
 *    thrust::xor_combine_engine<thrust::minstd_rand,0,thrust::minstd_rand0,0>
 * rng;
 *
 ...
 */
   ...
   */
            typedef typename thrust::detail::eval_if<
                (sizeof(typename base2_type::result_type)
                 > sizeof(typename base1_type::result_type)),
   ...
   */
            static const result_type max
                = detail::xor_combine_engine_max<Engine1, s1, Engine2, s2, result_type>::value;
   ...
   *  given \p base1_type and \p base2_type engines to initialize its adapted
   * base engines.
   *
   *  \param urng1 A \p base1_type to use to initialize this \p
   * xor_combine_engine's first adapted base engine. \param urng2 A \p
   * base2_type to use to initialize this \p xor_combine_engine's first adapted
   * base engine.
   */
            __host__ __device__ xor_combine_engine(const base1_type& urng1,
                                                   const base2_type& urng2);

            /*! This constructor initializes a new \p xor_combine_engine with a given
   * seed.
   *
   *  \param s The seed used to intialize this \p xor_combine_engine's adapted
   * base engines.
   */
            __host__ __device__ xor_combine_engine(result_type s);

            /*! This method initializes the state of this \p xor_combine_engine's adapted
   * base engines by using their \p default_seed values.
   */
            __host__ __device__ void seed(void);

            /*! This method initializes the state of this \p xor_combine_engine's adapted
   * base engines by using the given seed.
   *
   *  \param s The seed with which to intialize this \p xor_combine_engine's
   ...
   * xor_combine_engine's state. \return A new random number.
   */
            __host__ __device__ result_type operator()(void);

            /*! This member function advances this \p xor_combine_engine's state a given
   * number of times and discards the results.
   *
   ...
   */
            ...
            /*! This member function returns a const reference to this \p
   * xor_combine_engine's first adapted base engine.
   *
   *  \return A const reference to the first base engine this \p
   * xor_combine_engine adapts.
   */
            __host__ __device__ const base1_type& base1(void) const;

            /*! This member function returns a const reference to this \p
   * xor_combine_engine's second adapted base engine.
   *
   *  \return A const reference to the second base engine this \p
   * xor_combine_engine adapts.
   */
            __host__ __device__ const base2_type& base2(void) const;
   ...
   */
        private:
            ...
            __host__ __device__ bool equal(const xor_combine_engine& rhs) const;
