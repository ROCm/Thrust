...
namespace thrust
{
    ...
    namespace random
    {
        ...
        namespace detail
        {
            ...
            {
                __host__ __device__ static void discard(UIntType& state, unsigned long long z)
                {
                    for(; z > 0; --z)
                    {
            ...
            // XXX figure out a robust implemenation of this for any unsigned integer type
            // later
            template <thrust::detail::uint32_t a, thrust::detail::uint32_t m>
            struct linear_congruential_engine_discard_implementation<thrust::detail::uint32_t,
                                                                     a,
                                                                     0,
                                                                     m>
            {
                __host__ __device__ static void discard(thrust::detail::uint32_t& state,
                                                        unsigned long long        z)
                {
                    const thrust::detail::uint32_t modulus = m;

                    // XXX we need to use unsigned long long here or we will encounter overflow
                    // in the
                    //     multiplies below
                    ...
                    while(z > 0)
                    {
                        ...
                        // move to the next bit of the exponent, square (and mod) the base
                        // accordingly
                        z >>= 1;
                        multiplier = (multiplier * multiplier) % modulus;
                    }

                    state = static_cast<thrust::detail::uint32_t>((multiplier_to_z * state)
                                                                  % modulus);
                }
            }; // end linear_congruential_engine_discard
            ...
            struct linear_congruential_engine_discard
            {
                ...
                __host__ __device__ static void discard(LinearCongruentialEngine& lcg,
                                                        unsigned long long        z)
                {
                    ...
                    linear_congruential_engine_discard_implementation<result_type, a, c, m>::
                        discard(lcg.m_x, z);
                }
            }; // end linear_congruential_engine_discard
