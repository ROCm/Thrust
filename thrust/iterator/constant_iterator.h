 ...
 *  of constant values. This iterator is useful for creating a range filled with
 * the same value without explicitly storing it in memory. Using \p
 * constant_iterator saves both memory capacity and bandwidth.
 *
 *  The following code snippet demonstrates how to create a \p constant_iterator
 * whose \c value_type is \c int and whose value is \c 10.
 *
 *  \code
 ...
 *  same value. We will create a temporary \p constant_iterator with the
 * function \p make_constant_iterator function in order to avoid explicitly
 * specifying its type:
 *
 *  \code
 *  #include <thrust/iterator/constant_iterator.h>
 ...
 */
   ...
   */
        ...
        /*! Null constructor initializes this \p constant_iterator's constant using
   * its null constructor.
   */
        __host__ __device__ constant_iterator(void)
            : super_t()
            , m_value() {};

        /*! Copy constructor copies the value of another \p constant_iterator into
   * this \p constant_iterator.
   *
   ...
   */
        ...
        /*! Copy constructor copies the value of another \p constant_iterator with
   * related System type.
   *
   ...
   *  incrementable type related this this \p constant_iterator's respective
   * types.
   *
   *  \p v The value of this \p constant_iterator's constant value.
   *  \p i The index of this \p constant_iterator in a sequence. Defaults to the
   ...
   */
        ...
        /*! This method returns the value of this \p constant_iterator's constant
   * value. \return A \c const reference to this \p constant_iterator's constant
   * value.
   */
        __host__ __device__ Value const& value(void) const
        {
            return m_value;
        }
   ...
   */
    ...
    protected:
        __host__ __device__ Value const& value_reference(void) const
        {
            return m_value;
        }

        __host__ __device__ Value& value_reference(void)
        {
            return m_value;
        }
