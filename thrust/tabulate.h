 ...
 */
    ...
    /*! \p tabulate fills the range <tt>[first, last)</tt> with the value of a
 * function applied to each element's index.
 *
 *  For each iterator \c i in the range <tt>[first, last)</tt>, \p tabulate
 * performs the assignment <tt>*i = unary_op(i - first)</tt>.
 *
 ...
 * ForwardIterator's \c value_type, then <tt>T(0)</tt> is defined. \tparam
 * UnaryOperation is a model of <a
 ...
 * value_type.
 *
 *  The following code snippet demonstrates how to use \p tabulate to generate
 * the first \c n non-positive integers using the \p thrust::host execution
 ...
 *
 *  The following code snippet demonstrates how to use \p tabulate to generate
 * the first \c n non-positive integers:
 *
 *  \code
 *  #include <thrust/tabulate.h>
 *  #include <thrust/functional.h>
