 ...
 *  \brief \p thrust::system::cpp is the namespace containing functionality for
 * allocating, manipulating, and deallocating memory available to Thrust's
 * standard C++ backend system. The identifiers are provided in a separate
 * namespace underneath <tt>thrust::system</tt> for import convenience but are
 * also aliased in the top-level <tt>thrust::cpp</tt> namespace for easy access.
 *
 */
        namespace cpp
        {
 ...
 */
            ...
            /*! \p pointer stores a pointer to an object allocated in memory available to
 * the cpp system. This type provides type safety when dispatching standard
 * algorithms on ranges resident in cpp memory.
 *
 *  \p pointer has pointer semantics: it may be dereferenced and manipulated
 * with pointer arithmetic.
 *
 *  \p pointer can be created with the function \p cpp::malloc, or by explicitly
 * calling its constructor with a raw pointer.
 *
 *  The raw pointer encapsulated by a \p pointer may be obtained by eiter its
 * <tt>get</tt> member function or the \p raw_pointer_cast function.
 *
 *  \note \p pointer is not a "smart" pointer; it is the programmer's
 * responsibility to deallocate memory pointed to by \p pointer.
 *
 ...
 */
   ...
   */
            ...
            public:
                ...
                /*! \p pointer's no-argument constructor initializes its encapsulated pointer
   * to \c 0.
   */
                ...
                /*! This constructor allows construction of a <tt>pointer<const T></tt> from a
   * <tt>T*</tt>.
   *
   *  \param ptr A raw pointer to copy from, presumed to point to a location in
   * memory accessible by the \p cpp system. \tparam OtherT \p OtherT shall be
   * convertible to \p T.
   */
                ...
                /*! This constructor allows construction from another pointer-like object with
   * related type.
   *
   *  \param other The \p OtherPointer to copy.
   *  \tparam OtherPointer The system tag associated with \p OtherPointer shall
   * be convertible to \p thrust::system::cpp::tag and its element type shall be
   * convertible to \p T.
   */
                ...
                /*! Assignment operator allows assigning from another pointer-like object with
   * related type.
   *
 ...
 * to the \p cpp system. \p reference is the type of the result of dereferencing
 * a \p cpp::pointer.
 *
 ...
 */
   ...
   */
            ...
            private:
                typedef thrust::
                    reference<T, thrust::system::cpp::pointer<T>, thrust::system::cpp::reference<T>>
                        super_t;
   ...
   *  pointed to by the given \p pointer. After this \p reference is
   * constructed, it shall refer to the object pointed to by \p ptr.
   *
   ...
   */
                ...
                /*! This constructor accepts a const reference to another \p reference of
   * related type. After this \p reference is constructed, it shall refer to the
   * same object as \p other.
   *
   ...
   */
                template <typename OtherT>
                __host__ __device__ reference(const reference<OtherT>& other,
                ...
                /*! Copy assignment operator copy assigns from another \p reference of related
   * type.
   *
 ...
 *  \param ptr A <tt>cpp::pointer<void></tt> pointing to the beginning of an
 * area of memory previously allocated with <tt>cpp::malloc</tt>. \see
 * cpp::malloc \see std::free
 */
