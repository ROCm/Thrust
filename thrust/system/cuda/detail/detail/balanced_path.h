                ...
                namespace detail
                {
                    namespace balanced_path_detail
                    {
                        ...
                        template <bool UpperBound,
                                  ...
                                  typename T,
                                  typename Comp>
                        __host__ __device__ void BinarySearchIteration(
                            It data, int& begin, int& end, T key, int shift, Comp comp)
                        {
                        ...
                        __host__ __device__ int BinarySearch(It data, int count, T key, Comp comp)
                        {
                            ...
                            int end   = count;
                            while(begin < end)
                                BinarySearchIteration<UpperBound, int>(
                                    data, begin, end, key, 1, comp);
                            return begin;
                        }
                        ...
                        __host__ __device__ int
                                 BiasedBinarySearch(It data, int count, T key, IntT levels, Comp comp)
                        {
                            int begin = 0;
                            int end   = count;

                            if(levels >= 4 && begin < end)
                                BinarySearchIteration<UpperBound, IntT>(
                                    data, begin, end, key, 9, comp);
                            if(levels >= 3 && begin < end)
                                BinarySearchIteration<UpperBound, IntT>(
                                    data, begin, end, key, 7, comp);
                            if(levels >= 2 && begin < end)
                                BinarySearchIteration<UpperBound, IntT>(
                                    data, begin, end, key, 5, comp);
                            if(levels >= 1 && begin < end)
                                BinarySearchIteration<UpperBound, IntT>(
                                    data, begin, end, key, 4, comp);
                        ...
                        __host__ __device__ int
                                 MergePath(It1 a, int aCount, It2 b, int bCount, int diag, Comp comp)
                        {
                    ...
                    template <typename RandomAccessIterator1,
                              ...
                              typename Size2,
                              typename Compare>
                    __host__ __device__ thrust::pair<Size1, Size1>
                                        balanced_path(RandomAccessIterator1 first1,
                                                      Size1                 n1,
                    ...
                    {
                        typedef
                            typename thrust::iterator_traits<RandomAccessIterator1>::value_type T;

                        Size1 aIndex = balanced_path_detail::MergePath<false>(
                            first1, n1, first2, n2, diag, comp);
                        ...
                        if(bIndex < n2)
                        {
                            ...
                            Size1 aStart = balanced_path_detail::BiasedBinarySearch<false>(
                                first1, aIndex, x, levels, comp);
                            Size1 bStart = balanced_path_detail::BiasedBinarySearch<false>(
                                first2, bIndex, x, levels, comp);
                            ...
                            Size1 bRunEnd  = balanced_path_detail::BinarySearch<true>(
                                                first2 + bIndex, bEnd - bIndex, x, comp)
                                            + bIndex;
                            bRun = bRunEnd - bStart;
