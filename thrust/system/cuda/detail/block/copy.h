            ...
            namespace detail
            {
                namespace block
                {
                    ...
                    namespace trivial_copy_detail
                    {
                        ...
                        inline __device__ thrust::pair<Size, Size> quotient_and_remainder(Size n,
                                                                                          Size d)
                        {
                            Size quotient  = n / d;
                        ...
                        __device__ __thrust_forceinline__ void aligned_copy(
                            Context context, T* dst, const T* src, unsigned int num_elements)
                        {
                            for(unsigned int i = context.thread_index(); i < num_elements;
                    ...
                    __device__ __thrust_forceinline__ void trivial_copy(Context     context,
                                                                        void*       destination_,
                                                                        const void* source_,
                                                                        size_t      num_bytes)
                    {
                        ...
                        //     2. go hog wild
                        //     3. get the remainder
                        if(reinterpret_cast<size_t>(destination) % sizeof(uint2) != 0
                           || reinterpret_cast<size_t>(source) % sizeof(uint2) != 0)
                        {
                            for(unsigned int i = context.thread_index(); i < num_bytes;
                                i += context.block_dimension())
                            {
                        ...
                        else
                        {
                            ...
                            const thrust::pair<size_t, size_t> num_wide_elements_and_remainder_bytes
                                = trivial_copy_detail::quotient_and_remainder(num_bytes,
                                                                              sizeof(int2));

                            // copy int2 elements
                            trivial_copy_detail::aligned_copy(
                                context,
                            ...
                            const char* remainder_first = reinterpret_cast<const char*>(
                                source
                                + sizeof(int2) * num_wide_elements_and_remainder_bytes.first);
                            char* remainder_result = reinterpret_cast<char*>(
                                destination
                                + sizeof(int2) * num_wide_elements_and_remainder_bytes.first);
                            ...
                            trivial_copy_detail::aligned_copy(
                                ...
                                remainder_result,
                                remainder_first,
                                num_wide_elements_and_remainder_bytes.second);
                        }
                    } // end trivial_copy()
                    ...
                    namespace detail
                    {
                        namespace dispatch
                        {
                            ...
                            __thrust_forceinline__ __device__ RandomAccessIterator2
                            ...
                            {
                                typedef
                                    typename thrust::iterator_value<RandomAccessIterator1>::type T;

                                const T* src = &thrust::raw_reference_cast(*first);
                                T*       dst = &thrust::raw_reference_cast(*result);

                                size_t n = (last - first);
                                thrust::system::cuda::detail::block::trivial_copy(
                                    context, dst, src, n * sizeof(T));
                                return result + n;
                            } // end copy()
                                                                   ...
                                                                   thrust::detail::false_type is_trivial_copy)
                            {
                                ...
                                for(; first < last; first += context.block_dimension(),
                                                    result += context.block_dimension())
                                {
                                    thrust::raw_reference_cast(*result)
                                        = thrust::raw_reference_cast(*first);
                                } // end for
...
#else
                            typename thrust::detail::dispatch::is_trivial_copy<
                    ...
                    template <typename Context,
                              ...
                              typename Size,
                              typename RandomAccessIterator2>
                    inline __device__ RandomAccessIterator2
                                      async_copy_n(Context&              ctx,
                                                   RandomAccessIterator1 first,
                                                   Size                  n,
                                                   RandomAccessIterator2 result)
                    {
                        ...
                        {
                            thrust::raw_reference_cast(result[i])
                                = thrust::raw_reference_cast(first[i]);
                        }
                    ...
                    template <typename Context,
                              ...
                              typename Size,
                              typename RandomAccessIterator2>
                    inline __device__ RandomAccessIterator2 copy_n(Context&              ctx,
                                                                   RandomAccessIterator1 first,
                                                                   Size                  n,
                                                                   RandomAccessIterator2 result)
                    {
                    ...
                    template <unsigned int work_per_thread,
                              ...
                              typename RandomAccessIterator2>
                    inline __device__ RandomAccessIterator2
                                      async_copy_n_global_to_shared(Context&              ctx,
                                                                    RandomAccessIterator1 first,
                                                                    Size                  n,
                                                                    RandomAccessIterator2 result)
                    {
                        typedef
                            typename thrust::iterator_value<RandomAccessIterator1>::type value_type;
                    ...
                    __device__ RandomAccessIterator2
                               copy_n_global_to_shared(Context&              ctx,
                                                       RandomAccessIterator1 first,
                                                       Size                  n,
                                                       RandomAccessIterator2 result)
                    {
