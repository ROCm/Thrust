 ...
 */
                inline __host__ __device__ std::size_t block_size_with_maximum_potential_occupancy(
                    const function_attributes_t& attributes, const device_properties_t& properties);
 ...
 */
                ...
                template <typename UnaryFunction>
                inline __host__ __device__ size_t max_blocksize_subject_to_smem_usage(
                    const device_properties_t&   properties,
                    const function_attributes_t& attributes,
                ...
                namespace cuda_launch_config_detail
                {
                        ...
                        size_t                       dynamic_smem_bytes)
                    {
                        ...
                        // Calc limits
                        const size_t ctaLimitThreads
                            = (CTA_SIZE <= size_t(properties.maxThreadsPerBlock))
                        ...
                        if(properties.major <= 1)
                        {
                            ...
                            const size_t regsPerCTA = util::round_i(
                                attributes.numRegs * properties.hipWarpSize * numWarps,
                                regAllocationUnit);
                    ...
                    UnaryFunction                blocksize_to_dynamic_smem_usage)
                {
                    ...
                    {
                        size_t total_smem_usage = blocksize_to_dynamic_smem_usage(blocksize)
