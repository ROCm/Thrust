...
namespace bulk
{
    ...
    __device__ RandomAccessIterator2 adjacent_difference(bulk::agent<grainsize>& exec,
    ...
    __device__ RandomAccessIterator2
               adjacent_difference(bulk::concurrent_group<bulk::agent<grainsize_>, groupsize>& g,
                                   RandomAccessIterator1                                       first,
                                   RandomAccessIterator1                                       last,
                                   RandomAccessIterator2                                       result,
                                   T                                                           init,
                                   BinaryOperation                                             binary_op)
    {
        ...
        typedef typename bulk::concurrent_group<bulk::agent<grainsize_>, groupsize>::size_type
            size_type;
        ...
        for(; first < last; first += tile_size, result += tile_size)
        {
            ...
            size_type local_size
                = thrust::max(0, thrust::min<size_type>(grainsize, last - (first + local_offset)));

            // get the init for the next iteration
            T next_init
                = (first + local_offset + tile_size - 1 < last) ? first[tile_size - 1] : init;
    ...
    template <std::size_t groupsize,
              ...
              typename BinaryOperation>
    __device__ RandomAccessIterator2
               adjacent_difference(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& g,
                                   RandomAccessIterator1                                      first,
                                   RandomAccessIterator1                                      last,
                                   RandomAccessIterator2                                      result,
                                   BinaryOperation                                            binary_op)
