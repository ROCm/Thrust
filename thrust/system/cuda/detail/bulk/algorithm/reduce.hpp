...
namespace bulk
{
    ...
    __forceinline__ __device__ T reduce(const bulk::bounded<bound, bulk::agent<grainsize>>& exec,
    ...
    namespace detail
    {
        namespace reduce_detail
        {
            ...
            template <typename ConcurrentGroup,
                      ...
                      typename T,
                      typename BinaryFunction>
            __device__ T destructive_reduce_n(ConcurrentGroup&     g,
                                              RandomAccessIterator first,
                                              Size                 n,
                                              T                    init,
                                              BinaryFunction       binary_op)
    ...
    template <std::size_t groupsize,
              ...
              typename T,
              typename BinaryFunction>
    __device__ T reduce(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& g,
                        RandomAccessIterator                                       first,
                        RandomAccessIterator                                       last,
                        T                                                          init,
                        BinaryFunction                                             binary_op)
    {
        ...
        for(size_type offset = 0; offset < n;
            first += elements_per_group, offset += elements_per_group)
        {
            ...
            strided_iterator<RandomAccessIterator, size_type> local_first
                = make_strided_iterator(first + tid, static_cast<size_type>(groupsize));
            ...
            this_sum = this_sum_defined ? bulk::reduce(bulk::bound<grainsize>(g.this_exec),
                                                       local_inputs,
                                                       local_inputs + local_size,
                                                       this_sum,
                                                       binary_op)
                                        : bulk::reduce(bulk::bound<grainsize - 1>(g.this_exec),
                                                       local_inputs + 1,
                                                       local_inputs + local_size,
                                                       T(local_inputs[0]),
                                                       binary_op);
        ...
        T result = bulk::detail::reduce_detail::destructive_reduce_n(
            g, buffer, thrust::min<size_type>(groupsize, n), init, binary_op);
    ...
    __device__ T reduce(bulk::concurrent_group<>& g,
                        RandomAccessIterator      first,
                        RandomAccessIterator      last,
                        T                         init,
                        BinaryFunction            binary_op)
    {
        ...
        T result = detail::reduce_detail::destructive_reduce_n(
            g, buffer, thrust::min<size_type>(g.size(), n), init, binary_op);
