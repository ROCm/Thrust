...
namespace bulk
{
    ...
    template <std::size_t bound,
              ...
              typename Compare>
    __forceinline__ __device__ void stable_sort_by_key(const bounded<bound, agent<grainsize>>& exec,
    ...
    namespace detail
    {
        namespace stable_merge_sort_detail
        {
            ...
            __device__ typename thrust::detail::enable_if<bound <= groupsize * grainsize>::type
                inplace_merge_adjacent_partitions(
                    bulk::bounded<bound, bulk::concurrent_group<bulk::agent<grainsize>, groupsize>>&
                            g,
                    KeyType local_keys[grainsize],
                    ValType local_values[grainsize],
                    void*   stage_ptr,
                    int     count,
                    int     local_size,
                    Compare comp)
            {
                    ...
                    num_agents_per_merge *= 2)
                {
                    ...
                    size_type partition_first2
                        = thrust::min<size_type>(count, partition_first1 + input_size);
                    size_type partition_last2
                        = thrust::min<size_type>(count, partition_first2 + input_size);

                    size_type n1 = partition_first2 - partition_first1;
                    size_type n2 = partition_last2 - partition_first2;

                    size_type mp = bulk::merge_path(stage.keys + partition_first1,
                                                    n1,
                                                    stage.keys + partition_first2,
                                                    n2,
                                                    diag,
                                                    comp);
                    ...
                    bulk::merge_by_key(
                        bulk::bound<grainsize>(g.this_exec),
                        stage.keys + partition_first1 + mp,
                        stage.keys + partition_first2,
                        stage.keys + partition_first2 + diag - mp,
                        stage.keys + partition_last2,
                    ...
                    bulk::copy_n(bulk::bound<grainsize>(g.this_exec),
                                 local_values,
                                 local_size,
                                 stage.vals + local_offset);

                    // gather values into registers
                    bulk::gather(bulk::bound<grainsize>(g.this_exec),
                                 gather_indices,
                                 gather_indices + local_size,
                                 stage.vals,
                                 local_values);
                ...
                Compare               comp)
        {
            ...
            stage.keys = static_cast<key_type*>(
                bulk::malloc(g, tile_size * thrust::max(sizeof(key_type), sizeof(value_type))));
#else
            __shared__ union
            {
            ...
            bulk::copy_n(bulk::bound<grainsize>(g.this_exec),
                         stage.values + local_offset,
                         local_size,
                         local_values);

            // each agent sorts its local partition of the array
            bulk::stable_sort_by_key(bulk::bound<grainsize>(g.this_exec),
                                     local_keys,
                                     local_keys + local_size,
                                     local_values,
                                     comp);
            ...
            if(n == tile_size)
            {
                stable_merge_sort_detail::inplace_merge_adjacent_partitions(
                    g, local_keys, local_values, stage.keys, tile_size, grainsize, comp);
            } // end if
            else
            {
                stable_merge_sort_detail::inplace_merge_adjacent_partitions(
                    g, local_keys, local_values, stage.keys, n, local_size, comp);
            } // end else
