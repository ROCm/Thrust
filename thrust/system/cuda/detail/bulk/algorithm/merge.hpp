...
namespace bulk
{
    ...
    template <typename RandomAccessIterator1,
              ...
              typename Compare>
    __device__ Size merge_path(RandomAccessIterator1 first1,
                               Size                  n1,
    ...
    template <std::size_t bound,
              ...
              typename Compare>
    __device__ OutputIterator merge(const bulk::bounded<bound, agent<grainsize>>& e,
    ...
    template <std::size_t bound,
              ...
              typename Compare>
    __device__ thrust::pair<RandomAccessIterator5, RandomAccessIterator6>
               merge_by_key(const bulk::bounded<bound, bulk::agent<grainsize>>&,
                            RandomAccessIterator1 keys_first1,
                            RandomAccessIterator1 keys_last1,
                            RandomAccessIterator2 keys_first2,
                            RandomAccessIterator2 keys_last2,
    ...
    template <std::size_t bound,
              ...
              typename Compare>
    __device__ typename thrust::detail::enable_if<(bound <= groupsize * grainsize)>::type
        inplace_merge(
            bulk::bounded<bound, bulk::concurrent_group<bulk::agent<grainsize>, groupsize>>& g,
    ...
    {
        typedef
            typename bulk::concurrent_group<bulk::agent<grainsize>, groupsize>::size_type size_type;
        ...
        size_type local_size
            = thrust::max<size_type>(0, thrust::min<size_type>(grainsize, n1 + n2 - local_offset));
        bulk::copy_n(
            bulk::bound<grainsize>(g.this_exec), local_result, local_size, first + local_offset);
    ...
    template <std::size_t bound,
              ...
              typename RandomAccessIterator3,
              typename Compare>
    __device__ typename thrust::detail::enable_if<(bound <= groupsize * grainsize),
                                                  RandomAccessIterator3>::type
        merge(bulk::bounded<bound, bulk::concurrent_group<bulk::agent<grainsize>, groupsize>>& g,
              ...
              Compare               comp)
    {
        ...
        value_type local_result[grainsize];
        bulk::merge(bulk::bound<grainsize>(g.this_exec),
                    first1 + local_offset1,
                    last1,
                    first2 + local_offset2,
                    last2,
                    local_result,
                    comp);
        ...
        size_type local_size
            = thrust::max<size_type>(0, thrust::min<size_type>(grainsize, n1 + n2 - local_offset));
        bulk::copy_n(
            bulk::bound<grainsize>(g.this_exec), local_result, local_size, result + local_offset);
    ...
    namespace detail
    {
        namespace merge_detail
        {
            ...
            template <std::size_t groupsize,
                      ...
                      typename RandomAccessIterator4,
                      typename Compare>
            __device__ RandomAccessIterator4 bounded_merge_with_buffer(
                bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& exec,
                RandomAccessIterator1                                      first1,
                RandomAccessIterator1                                      last1,
                RandomAccessIterator2                                      first2,
                RandomAccessIterator2                                      last2,
    ...
    template <std::size_t groupsize,
              ...
              typename RandomAccessIterator3,
              typename Compare>
    __device__ RandomAccessIterator3
               merge(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& exec,
                     RandomAccessIterator1                                      first1,
                     RandomAccessIterator1                                      last1,
                     RandomAccessIterator2                                      first2,
                     RandomAccessIterator2                                      last2,
                     ...
                     Compare                                                    comp)
    {
        ...
        value_type* buffer = reinterpret_cast<value_type*>(
            bulk::malloc(exec, exec.size() * exec.grainsize() * sizeof(value_type)));
        ...
        if(n1 + n2 <= chunk_size)
        {
            result = detail::merge_detail::bounded_merge_with_buffer(
                exec, first1, last1, first2, last2, buffer, result, comp);
        } // end if
        else
        {
            while((first1 < last1) || (first2 < last2))
            {
                ...
                result = detail::merge_detail::bounded_merge_with_buffer(
                    exec, first1, first1 + mp, first2, first2 + diag - mp, buffer, result, comp);
    ...
    template <std::size_t groupsize,
              ...
              typename Compare>
    __device__ thrust::pair<RandomAccessIterator5, RandomAccessIterator6>
               merge_by_key(bulk::bounded<groupsize * grainsize,
                                   bulk::concurrent_group<bulk::agent<grainsize>, groupsize>>& g,
                            RandomAccessIterator1 keys_first1,
                            RandomAccessIterator1 keys_last1,
                            RandomAccessIterator2 keys_first2,
                            RandomAccessIterator2 keys_last2,
                            ...
                            Compare               comp)
    {
        ...
        stage.keys = static_cast<key_type*>(bulk::malloc(
            g, groupsize * grainsize * thrust::max(sizeof(key_type), sizeof(size_type))));
#else
        __shared__ union
        {
        ...
        bulk::copy_n(
            g, thrust::detail::make_join_iterator(keys_first1, n1, keys_first2), n, stage.keys);
        ...
        size_type local_offset = grainsize * g.this_exec.index();
        size_type local_size
            = thrust::max<size_type>(0, thrust::min<size_type>(grainsize, n - local_offset));
        bulk::copy_n(
            bulk::bound<grainsize>(g.this_exec), results, local_size, stage.keys + local_offset);
        g.wait();
        ...
        bulk::copy_n(
            bulk::bound<grainsize>(g.this_exec), indices, local_size, stage.indices + local_offset);
        g.wait();
