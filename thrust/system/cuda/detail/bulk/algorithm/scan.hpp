...
namespace bulk
{
    ...
    __forceinline__ __device__ RandomAccessIterator2
    ...
    namespace detail
    {
        namespace scan_detail
        {
            ...
            template <typename ConcurrentGroup,
                      ...
                      typename T,
                      typename BinaryFunction>
            __device__ T inplace_exclusive_scan(ConcurrentGroup&     g,
            ...
            template <typename ConcurrentGroup,
                      ...
                      typename T,
                      typename BinaryFunction>
            __device__ T small_inplace_exclusive_scan(ConcurrentGroup&     g,
                                                      RandomAccessIterator first,
                                                      Size                 n,
                                                      T                    init,
                                                      BinaryFunction       binary_op)
            ...
            template <typename ConcurrentGroup,
                      ...
                      typename T,
                      typename BinaryFunction>
            __device__ T bounded_inplace_exclusive_scan(ConcurrentGroup&     g,
                                                        RandomAccessIterator first,
                                                        Size                 n,
                                                        T                    init,
                                                        BinaryFunction       binary_op)
            {
                return (n == g.size()) ? inplace_exclusive_scan(g, first, init, binary_op)
                     ...
                     BinaryFunction        binary_op)
            {
                ...
                typedef typename scan_intermediate<RandomAccessIterator1,
                                                   RandomAccessIterator2,
                                                   BinaryFunction>::type intermediate_type;

                typedef typename bulk::bounded<
                    bound,
                    bulk::concurrent_group<bulk::agent<grainsize>, groupsize>>::size_type size_type;
                ...
                size_type local_size   = thrust::max<size_type>(
                    0, thrust::min<size_type>(grainsize, n - grainsize * tid));
                ...
                {
                    x = local_inputs[0];
                    x = bulk::accumulate(bulk::bound<grainsize - 1>(g.this_exec),
                                         local_inputs + 1,
                ...
                const size_type spine_n
                    = (n >= g.size() * g.this_exec.grainsize())
                          ? g.size()
                          : (n + g.this_exec.grainsize() - 1) / g.this_exec.grainsize();
                ...
                if(inclusive)
                {
                    bulk::inclusive_scan(bulk::bound<grainsize>(g.this_exec),
                                         local_inputs,
                                         local_inputs + local_size,
                                         result + local_offset,
                                         x,
                ...
                {
                    bulk::exclusive_scan(bulk::bound<grainsize>(g.this_exec),
                                         local_inputs,
                                         local_inputs + local_size,
                                         result + local_offset,
                                         x,
            ...
            template <bool        inclusive,
                      ...
                      typename BinaryFunction>
            __device__ void
                scan_with_buffer(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& g,
                                 RandomAccessIterator1                                      first,
                                 RandomAccessIterator1                                      last,
                                 RandomAccessIterator2                                      result,
                                 T                            carry_in,
                                 BinaryFunction               binary_op,
                                 scan_buffer<groupsize,
                                             grainsize,
                                             RandomAccessIterator1,
                                             RandomAccessIterator2,
                                             BinaryFunction>& buffer)
            {
                typedef scan_buffer<groupsize,
                                    grainsize,
                                    RandomAccessIterator1,
                                    RandomAccessIterator2,
                                    BinaryFunction>
    ...
    __device__ void inclusive_scan(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& g,
                                   ...
                                   BinaryFunction        binary_op)
    {
        typedef detail::scan_detail::scan_buffer<groupsize,
                                                 grainsize,
                                                 RandomAccessIterator1,
                                                 RandomAccessIterator2,
                                                 BinaryFunction>
        ...
        {
            detail::scan_detail::scan_with_buffer<true>(
                g, first, last, result, init, binary_op, *bulk::on_chip_cast(buffer));
        } // end if
        else
        {
            detail::scan_detail::scan_with_buffer<true>(
                g, first, last, result, init, binary_op, *buffer);
        } // end else
        ...
        detail::scan_detail::scan_with_buffer<true>(
            g, first, last, result, init, binary_op, buffer.get());
#endif // __CUDA_ARCH__
    } // end inclusive_scan()
    ...
    template <std::size_t size,
              ...
              typename RandomAccessIterator2,
              typename BinaryFunction>
    __device__ RandomAccessIterator2
                              ...
                              BinaryFunction                                        binary_op)
    {
        if(first < last)
        {
            ...
            //typename thrust::iterator_value<RandomAccessIterator1>::type init = *first;
            typename detail::scan_detail::scan_intermediate<RandomAccessIterator1,
                                                            RandomAccessIterator2,
                                                            BinaryFunction>::type init
                = *first;
    ...
    template <std::size_t groupsize,
              ...
              typename T,
              typename BinaryFunction>
    __device__ typename thrust::detail::enable_if<(groupsize > 0), RandomAccessIterator2>::type
                       ...
                       BinaryFunction                                       binary_op)
    {
        ...
        if(bulk::is_on_chip(buffer))
        {
            detail::scan_detail::scan_with_buffer<false>(
                g, first, last, result, init, binary_op, *bulk::on_chip_cast(buffer));
        } // end if
        else
        {
