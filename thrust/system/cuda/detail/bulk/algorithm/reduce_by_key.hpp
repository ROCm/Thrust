...
namespace bulk
{
    namespace detail
    {
        namespace reduce_by_key_detail
        {
            ...
            struct scan_head_flags_functor
            {
                ...
                __host__ __device__ scan_head_flags_functor(BinaryFunction binary_op)
                    : binary_op(binary_op)
                {
                }

                __host__ __device__ result_type operator()(const first_argument_type&  a,
                                                           const second_argument_type& b)
                {
                    ValueType val = thrust::get<0>(b)
                                        ? thrust::get<1>(b)
                                        : binary_op(thrust::get<1>(a), thrust::get<1>(b));
            ...
            __device__ void scatter_tails_n(ConcurrentGroup& group,
    ...
    thrust::tuple<OutputIterator1,
        ...
        __device__ reduce_by_key(bulk::concurrent_group<bulk::agent<grainsize>, groupsize>& g,
                                 ...
                                 BinaryFunction  binary_op)
    {
        typedef typename thrust::iterator_value<InputIterator2>::type
            value_type; // XXX this should be the type returned by BinaryFunction
...
#if __CUDA_ARCH__ >= 200
        size_type* s_flags
            = reinterpret_cast<size_type*>(bulk::malloc(g, interval_size * sizeof(int)));
        value_type* s_values
            = reinterpret_cast<value_type*>(bulk::malloc(g, interval_size * sizeof(value_type)));
#else
        ...
        for(; keys_first < keys_last; keys_first += interval_size, values_first += interval_size)
        {
            ...
            bulk::detail::head_flags_with_init<InputIterator1, BinaryPredicate, size_type> flags(
                keys_first, keys_first + n, init_key, pred);

            detail::reduce_by_key_detail::
                scan_head_flags_functor<size_type, value_type, BinaryFunction>
                    f(binary_op);
