...
namespace bulk
{
    ...
    inline __device__ T* on_chip_cast(T* ptr)
    {
        ...
        HIP_DYNAMIC_SHARED(char, s_begin)
    ...
    namespace detail
    {
        ...
        private:
            ...
            {
                return reinterpret_cast<char*>(m_program_break)
                       - reinterpret_cast<char*>(s_data_segment_begin);
        ...
        public:
            ...
            __device__ inline void* allocate(size_t size)
            {
                ...
                block* prev
                    = find_first_free_insertion_point(heap_begin(), heap_end(), aligned_size);
        ...
        private:
            ...
            __device__ inline static block*
                find_first_free_insertion_point(block* first, block* last, size_t size)
            {
                block* prev = last;
        ...
        public:
            ...
            inline __device__ void* unsafe_allocate(size_t size)
            {
                return m_alloc.allocate(size);
            }

            inline __device__ void* allocate(size_t size)
            {
                void* result;
            ...
            inline __device__ void unsafe_deallocate(void* ptr)
            {
                m_alloc.deallocate(ptr);
            } // end unsafe_deallocate()

            inline __device__ void deallocate(void* ptr)
            {
                m_mutex.lock();
                {
        ...
        private:
            ...
            {
            public:
                inline __device__ mutex()
                    : m_in_use(0)
                {
                }

                inline __device__ bool try_lock()
                {
...
// put the object in an anonymous namespace so that non-CUDA compilers don't complain about multiple definitions
    ...
    __device__ inline void* malloc(ConcurrentGroup& g, size_t num_bytes)
    {
    ...
    template <typename ConcurrentGroup>
    __device__ inline void free(ConcurrentGroup& g, void* ptr)
    {
        if(g.this_exec.index() == 0)
        {
