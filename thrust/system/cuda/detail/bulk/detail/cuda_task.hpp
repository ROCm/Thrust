...
namespace bulk
{
    namespace detail
    {
        ...
        public:
            ...
            __host__ __device__ task_base(group_type g, closure_type c)
                : c(c)
                , g(g)
            {
            }
        ...
        protected:
            __host__ __device__ static void substitute_placeholders_and_execute(group_type&   g,
                                                                                closure_type& c)
            {
                ...
                closure<typename closure_type::function_type, substituted_arguments_type> new_c(
                    c.function(), new_args);
        ...
        private:
            template <typename T>
            struct substitutor_result : thrust::detail::eval_if<bulk::detail::is_cursor<T>::value,
            ...
            typedef
                typename bulk::detail::tuple_meta_transform<typename closure_type::arguments_type,
                                                            substitutor_result>::type
                    substituted_arguments_type;

            struct substitutor
            {
                group_type& g;

                __device__ substitutor(group_type& g)
                    : g(g)
                {
                }

                template <unsigned int depth>
                __device__ typename bulk::detail::cursor_result<cursor<depth>, group_type>::type
            ...
            __host__ __device__ static substituted_arguments_type
                     substitute_placeholders(group_type& g, typename closure_type::arguments_type args)
            {
                return bulk::detail::tuple_host_device_transform<substitutor_result>(
                    args, substitutor(g));
            }
        };
        ...
        template <typename Group, typename Closure>
        class cuda_task;

        template <typename Grid>
        struct grid_maker
        {
            __host__ __device__ static Grid make(typename Grid::size_type  size,
        ...
        {
            __host__ __device__ static parallel_group<Block, dynamic_group_size>
                     make(typename parallel_group<Block, dynamic_group_size>::size_type size,
                          Block                                                         block,
        ...
        struct block_maker
        {
            __host__ __device__ static Block make(typename Block::size_type,
        ...
        {
            __host__ __device__ static concurrent_group<Thread, dynamic_group_size>
                     make(typename concurrent_group<Thread, dynamic_group_size>::size_type size,
        ...
        template <typename Grid>
        __host__ __device__ Grid make_grid(typename Grid::size_type  size,
                                           typename Grid::agent_type block,
                                           typename Grid::size_type  index = invalid_index)
        {
            return grid_maker<Grid>::make(size, block, index);
        }

        template <typename Block>
        __host__ __device__ Block make_block(typename Block::size_type  size,
                                             typename Block::size_type  heap_size,
                                             typename Block::agent_type thread
                                             = typename Block::agent_type(),
                                             typename Block::size_type index = invalid_index)
        {
        ...
        template <std::size_t gridsize,
                  ...
                  typename Closure>
        class cuda_task<parallel_group<concurrent_group<agent<grainsize>, blocksize>, gridsize>,
                        Closure>
            : public task_base<typename cuda_grid<gridsize, blocksize, grainsize>::type, Closure>
        {
        private:
            typedef task_base<typename cuda_grid<gridsize, blocksize, grainsize>::type, Closure>
                super_t;
        ...
        public:
            __host__ __device__ cuda_task(grid_type g, closure_type c, size_type offset)
                : super_t(g, c)
                , block_offset(offset)
            {
            }
            //added in trail
            __host__        __device__ ~cuda_task() {}
...
//#ifdef __CUDA_ARCH__
                ...
                grid_type this_grid
                    = make_grid<grid_type>(super_t::g.size(),
                                           make_block<block_type>(hipBlockDim_x,
                                                                  super_t::g.this_exec.heap_size(),
                                                                  thread_type(hipThreadIdx_x),
                                                                  block_offset + hipBlockIdx_x),
                                           0);
        ...
        class cuda_task<concurrent_group<agent<grainsize>, blocksize>, Closure>
        ...
        public:
            __host__ __device__ cuda_task(block_type b, closure_type c)
                : super_t(b, c)
            {
            }
...
//#ifdef __CUDA_ARCH__
                ...
                block_type this_block = make_block<block_type>(
                    hipBlockDim_x, super_t::g.heap_size(), thread_type(hipThreadIdx_x), 0);
        ...
        public:
            ...
            __host__ __device__ cuda_task(group_type g, closure_type c)
                : super_t(g, c)
            {
            }
