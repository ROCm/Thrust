...
namespace bulk
{
    namespace detail
    {
        ...
        public:
            ...
            __host__ __device__ explicit parameter_ptr(element_type* ptr)
                : m_ptr(ptr)
            {
            }

            // XXX copy emulates a move
            __host__ __device__ parameter_ptr(const parameter_ptr& other_)
            {
                parameter_ptr& other = const_cast<parameter_ptr&>(other_);
                thrust::swap(m_ptr, other.m_ptr);
            }

            __host__ __device__ ~parameter_ptr()
            {
            ...
            __host__ __device__ parameter_ptr& operator=(const parameter_ptr& other_)
            {
                parameter_ptr& other = const_cast<parameter_ptr&>(other_);
                thrust::swap(m_ptr, other.m_ptr);
                return *this;
        ...
        template <typename T>
        __host__ __device__ parameter_ptr<T> make_parameter(const T& x)
        {
            T* raw_ptr = 0;

            // allocate
#if __BULK_HAS_CUDART__
            bulk::detail::throw_on_error(hipMalloc(&raw_ptr, sizeof(T)),
                                         "make_parameter(): after hipMalloc");
#else
