...
namespace bulk
{
    namespace detail
    {
        ...
        struct cuda_launcher_base
            : public triple_chevron_launcher<block_size_, cuda_task<ExecutionGroup, Closure>>
        {
            typedef triple_chevron_launcher<block_size_, cuda_task<ExecutionGroup, Closure>>
                                                       super_t;
            typedef typename super_t::task_type        task_type;
            typedef typename ExecutionGroup::size_type size_type;

            __host__ __device__ cuda_launcher_base()
                : m_device_properties(bulk::detail::device_properties())
            {
            }

            __host__ __device__ void launch(size_type   num_blocks,
                                            size_type   block_size,
                                            size_type   num_dynamic_smem_bytes,
                                            hipStream_t stream,
                                            task_type   task)
            {
                if(num_blocks > 0)
            ...
            __host__ __device__ static size_type
                     max_active_blocks_per_multiprocessor(const device_properties_t&   props,
                                                          const function_attributes_t& attr,
                                                          size_type num_threads_per_block,
                                                          size_type num_smem_bytes_per_block)
            {
                return static_cast<size_type>(
                    bulk::detail::cuda_launch_config_detail::max_active_blocks_per_multiprocessor(
                        props, attr, num_threads_per_block, num_smem_bytes_per_block));
            ...
            // 2. kernel occupancy
            __host__ __device__ static thrust::pair<size_type, size_type>
                     dynamic_smem_occupancy_limit(const device_properties_t&   props,
                                                  const function_attributes_t& attr,
                                                  size_type                    num_threads_per_block,
                                                  size_type                    num_smem_bytes_per_block)
            {
                // figure out the kernel's occupancy with 0 bytes of dynamic smem
                size_type occupancy = max_active_blocks_per_multiprocessor(
                    props, attr, num_threads_per_block, num_smem_bytes_per_block);
                ...
                return thrust::make_pair(
                    static_cast<size_type>(
                        bulk::detail::proportional_smem_allocation(props, attr, occupancy)),
                    occupancy);
            } // end smem_occupancy_limit()

            __host__ __device__ size_type choose_heap_size(const device_properties_t& props,
                                                           size_type                  group_size,
                                                           size_type requested_size)
            {
                function_attributes_t attr
                    = bulk::detail::function_attributes(super_t::global_function_pointer());
                ...
                thrust::tie(result, occupancy)
                    = dynamic_smem_occupancy_limit(props, attr, group_size, 0);
                ...
                if(requested_size != use_default && requested_size > result && occupancy > 1)
                {
                    ...
                    if(requested_size > result)
                    {
                        ...
                        thrust::tie(next_level_result, next_level_occupancy)
                            = dynamic_smem_occupancy_limit(props, attr, group_size, requested_size);

                        // if we didn't completely overflow things, use this new heap size
                        // otherwise, the heap remains the default size
                        if(next_level_occupancy > 0)
                            result = next_level_result;
                    } // end else
                } // end i
            ...
            __host__ __device__ size_type choose_group_size(size_type requested_size)
            {
                size_type result = requested_size;

                if(result == use_default)
                {
                    bulk::detail::function_attributes_t attr
                        = bulk::detail::function_attributes(super_t::global_function_pointer());

                    return static_cast<size_type>(
                        bulk::detail::block_size_with_maximum_potential_occupancy(
                            attr, device_properties()));
                } // end if

                return result;
            } // end choose_group_size()

            __host__ __device__ size_type choose_subscription(size_type block_size)
            {
                // given no other info, this is a reasonable guess
                return block_size > 0 ? device_properties().maxThreadsPerMultiProcessor / block_size
                                      : 0;
            }

            __host__ __device__ size_type choose_num_groups(size_type requested_num_groups,
                                                            size_type group_size)
            {
                ...
                if(result == use_default)
                {
                    ...
                    result = thrust::min<size_type>(subscription
            ...
            __host__ __device__ size_type max_physical_grid_size()
            {
            ...
            __host__ __device__ const device_properties_t& device_properties() const
            {
        ...
        template <std::size_t gridsize,
                  std::size_t blocksize,
                  std::size_t grainsize,
                  typename Closure>
        struct cuda_launcher<
            parallel_group<concurrent_group<agent<grainsize>, blocksize>, gridsize>,
            Closure>
            : public cuda_launcher_base<blocksize,
                                        typename cuda_grid<gridsize, blocksize, grainsize>::type,
                                        Closure>
        {
            typedef cuda_launcher_base<blocksize,
                                       typename cuda_grid<gridsize, blocksize, grainsize>::type,
                                       Closure>
                                                super_t;
            ...
            __host__ __device__ void launch(grid_type request, Closure c, hipStream_t stream)
            {
                ...
                if(num_blocks > 0 && block_size > 0)
                {
                    ...
                    if(block_size > 0)
                    {
                        ...
                        for(size_type block_offset = 0; block_offset < num_blocks;
                            block_offset += max_physical_grid_size)
                        {
                            task_type task(g, c, block_offset);

                            size_type num_physical_blocks = thrust::min<size_type>(
                                num_remaining_physical_blocks, max_physical_grid_size);

                            super_t::launch(
                                num_physical_blocks, block_size, heap_size, stream, task);
            ...
            __host__ __device__ grid_type configure(grid_type g)
            {
                size_type block_size = super_t::choose_group_size(g.this_exec.size());
                size_type heap_size  = super_t::choose_heap_size(
                    device_properties(), block_size, g.this_exec.heap_size());
                size_type num_blocks = g.size();

                return make_grid<grid_type>(num_blocks,
                                            make_block<block_type>(block_size, heap_size));
            } // end configure()

            // chooses a number of groups and a group size
            __host__ __device__ thrust::pair<size_type, size_type>
                                choose_sizes(size_type requested_num_groups, size_type requested_group_size)
            {
        ...
        struct cuda_launcher<concurrent_group<agent<grainsize>, blocksize>, Closure>
            : public cuda_launcher_base<blocksize,
                                        concurrent_group<agent<grainsize>, blocksize>,
                                        Closure>
        {
            typedef cuda_launcher_base<blocksize,
                                       concurrent_group<agent<grainsize>, blocksize>,
            ...
            __host__ __device__ void launch(block_type request, Closure c, hipStream_t stream)
            {
            ...
            __host__ __device__ block_type configure(block_type b)
            {
                size_type block_size = super_t::choose_group_size(b.size());
                size_type heap_size
                    = super_t::choose_heap_size(device_properties(), block_size, b.heap_size());
                return make_block<block_type>(block_size, heap_size);
            } // end configure()
        }; // end cuda_launcher

        template <std::size_t groupsize, std::size_t grainsize, typename Closure>
        struct cuda_launcher<parallel_group<agent<grainsize>, groupsize>, Closure>
            : public cuda_launcher_base<dynamic_group_size,
                                        parallel_group<agent<grainsize>, groupsize>,
                                        Closure>
        {
            typedef cuda_launcher_base<dynamic_group_size,
                                       parallel_group<agent<grainsize>, groupsize>,
                                       Closure>
                                                super_t;
            typedef typename super_t::size_type size_type;
            ...
            __host__ __device__ void launch(group_type g, Closure c, hipStream_t stream)
            {
            ...
            __host__ __device__ thrust::tuple<size_type, size_type> configure(group_type g)
            {
                size_type block_size
                    = thrust::min<size_type>(g.size(), super_t::choose_group_size(use_default));
                ...
                size_type num_blocks
                    = (block_size > 0) ? (g.size() + block_size - 1) / block_size : 0;
