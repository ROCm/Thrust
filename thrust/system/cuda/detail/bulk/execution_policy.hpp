...
class async_launch
{
  public:
    __host__ __device__
    async_launch(ExecutionAgent exec, hipStream_t s, hipEvent_t be = 0)
      : stream_valid(true),e(exec),s(s),be(be)
    {}

    __host__
    async_launch(ExecutionAgent exec, hipEvent_t be)
...
inline __host__ __device__
async_launch<bulk::parallel_group<> > par(hipStream_t s, size_t num_threads)
{
...
template<typename ExecutionAgent>
inline __host__ __device__
async_launch<bulk::parallel_group<ExecutionAgent> > par(hipStream_t s, ExecutionAgent exec, size_t num_groups)
{
