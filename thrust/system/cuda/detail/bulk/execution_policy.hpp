...
namespace bulk
{
    ...
    public:
        ...
        __host__ __device__ agent(size_type i = invalid_index)
            : m_index(i)
        {
        }

        __host__ __device__ size_type index() const
        {
            return m_index;
        }

        __host__ __device__ size_type grainsize() const
        {
    ...
    namespace detail
    {
        namespace group_detail
        {
            ...
            public:
                ...
                __host__ __device__ group_base(agent_type exec = agent_type(),
                ...
                __device__ size_type global_index() const
                {
            ...
            public:
                ...
                __host__ __device__ group_base(size_type  sz,
                ...
                __host__ __device__ size_type global_index() const
                {
                    return index() * size() + this_exec.index();
                }
    ...
    template <typename ExecutionAgent = agent<>, std::size_t size_ = dynamic_group_size>
    class parallel_group : public detail::group_detail::group_base<ExecutionAgent, size_>
    {
    private:
        typedef detail::group_detail::group_base<ExecutionAgent, size_> super_t;
    ...
    public:
        ...
        __host__ __device__ parallel_group(agent_type exec = agent_type(),
                                           size_type  i    = invalid_index)
            : super_t(exec, i)
        {
        }
    ...
    {
    private:
        typedef detail::group_detail::group_base<ExecutionAgent, dynamic_group_size> super_t;
    ...
    public:
        ...
        // XXX the constructor taking an index should be made private
        __host__ __device__ parallel_group(size_type  size,
                                           agent_type exec = agent_type(),
                                           size_type  i    = invalid_index)
            : super_t(size, exec, i)
        {
    ...
    inline __host__ __device__ parallel_group<> par(size_t size)
    {
    ...
    template <typename ExecutionAgent>
    __host__ __device__ parallel_group<ExecutionAgent> par(ExecutionAgent exec, size_t size)
    {
    ...
    {
    public:
        __host__ __device__ async_launch(ExecutionAgent exec, hipStream_t s, hipEvent_t be = 0)
            : stream_valid(true)
            , e(exec)
            , s(s)
            , be(be)
        ...
        __host__ async_launch(ExecutionAgent exec, hipEvent_t be)
            : stream_valid(false)
            , e(exec)
            , s(0)
            , be(be)
        ...
        __host__ __device__ ExecutionAgent exec() const
        {
            return e;
        }

        __host__ __device__ hipStream_t stream() const
        {
            return s;
        }

        __host__ __device__ hipEvent_t before_event() const
        {
            return be;
        }

        __host__ __device__ bool is_stream_valid() const
        {
            return stream_valid;
        }
    ...
    inline __host__ __device__ async_launch<bulk::parallel_group<>> par(hipStream_t s,
                                                                        size_t      num_threads)
    {
        typedef bulk::parallel_group<>::size_type size_type;
        return async_launch<bulk::parallel_group<>>(
            bulk::parallel_group<>(static_cast<size_type>(num_threads)), s);
    }

    template <typename ExecutionAgent>
    inline __host__ __device__ async_launch<bulk::parallel_group<ExecutionAgent>>
                               par(hipStream_t s, ExecutionAgent exec, size_t num_groups)
    {
    ...
    {
        hipEvent_t before_event = bulk::detail::future_core_access::event(before);

        typedef bulk::parallel_group<>::size_type size_type;
        return async_launch<bulk::parallel_group<>>(
            bulk::parallel_group<>(static_cast<size_type>(num_threads)), before_event);
    }
    ...
    {
    private:
        typedef parallel_group<ExecutionAgent, size_> super_t;
    ...
    public:
        ...
        // XXX the constructor taking an index should be made private
        __host__ __device__ concurrent_group(size_type  heap_size = use_default,
                                             agent_type exec      = agent_type(),
                                             size_type  i         = invalid_index)
            : super_t(exec, i)
...
//#ifdef __CUDA_ARCH__
        ...
        __host__ __device__ size_type heap_size() const
        {
            return m_heap_size;
        }

        // XXX this should go elsewhere
        __host__ __device__ inline static size_type hardware_concurrency()
        {
    ...
    {
    private:
        typedef parallel_group<ExecutionAgent, dynamic_group_size> super_t;
    ...
    public:
        ...
        // XXX the constructor taking an index should be made private
        __host__ __device__ concurrent_group(size_type  size,
...
//#ifdef __CUDA_ARCH__
    ...
    inline __host__ __device__ concurrent_group<> con(size_t size, size_t heap_size = use_default)
    {
    ...
    template <typename ExecutionAgent>
    __host__ __device__ concurrent_group<ExecutionAgent>
                        con(ExecutionAgent exec, size_t size, size_t heap_size = use_default)
    {
        ...
        return concurrent_group<ExecutionAgent>(
            static_cast<size_type>(size), static_cast<size_type>(heap_size), exec);
    }

    // shorthand for creating a concurrent_group of agents with static sizing
    template <std::size_t groupsize, std::size_t grainsize>
    __host__ __device__ concurrent_group<bulk::agent<grainsize>, groupsize> con(size_t heap_size)
    {
        typedef typename concurrent_group<bulk::agent<grainsize>, groupsize>::size_type size_type;
        return concurrent_group<bulk::agent<grainsize>, groupsize>(
            static_cast<size_type>(heap_size));
    }
    ...
    public:
        ...
        __host__ __device__ ExecutionAgent& unbound()
        {
            return *this;
        }

        __host__ __device__ const ExecutionAgent& unbound() const
        {
            return *this;
        }
    ...
    template <std::size_t bound_, typename ExecutionAgent>
    __host__ __device__ bounded<bound_, ExecutionAgent>& bound(ExecutionAgent& exec)
    {
        return static_cast<bounded<bound_, ExecutionAgent>&>(exec);
    }

    template <std::size_t bound_, typename ExecutionAgent>
    __host__ __device__ const bounded<bound_, ExecutionAgent>& bound(const ExecutionAgent& exec)
    {
    ...
    namespace detail
    {
        ...
        {
            typedef typename agent_at_depth<depth - 1, ExecutionAgent>::type parent_agent_type;
        ...
        struct cursor
        {
            ...
            static __host__ __device__ typename cursor_result<cursor, ExecutionGroup>::type
        ...
        struct cursor<0>
        {
            ...
            template <typename ExecutionAgent>
            static __host__ __device__ ExecutionAgent& get(ExecutionAgent& root)
            {
                return root;
            }
        };
        ...
        template <typename T>
        struct is_cursor : thrust::detail::false_type
        {
        };

        template <unsigned int d>
        struct is_cursor<cursor<d>> : thrust::detail::true_type
        {
        };
...
//#ifdef __CUDA_ARCH__
    ...
    inline __host__ __device__ parallel_group<concurrent_group<>>
                               grid(size_t num_groups = use_default,
                                    size_t group_size = use_default,
                                    size_t heap_size  = use_default)
    {
        return par(con(group_size, heap_size), num_groups);
    }

    inline __host__ __device__ async_launch<parallel_group<concurrent_group<>>>
                               grid(size_t num_groups, size_t group_size, size_t heap_size, hipStream_t stream)
    {
        return par(stream, con(group_size, heap_size), num_groups);
    }

    template <std::size_t groupsize, std::size_t grainsize>
    __host__ __device__ parallel_group<concurrent_group<bulk::agent<grainsize>, groupsize>>
    ...
    template <std::size_t groupsize, std::size_t grainsize>
    __host__
        __device__ async_launch<parallel_group<concurrent_group<bulk::agent<grainsize>, groupsize>>>
                   grid(size_t num_groups, size_t heap_size, hipStream_t stream)
    {
