...
/*! \file error_code.h
 *  \brief An object used to hold error values, such as those originating from
 ...
 */
        ...
        /*! A metafunction returning whether or not the parameter is an \p error_code
 * enum.
 */
        template <typename T>
        ...
        /*! A metafunction returning whether or not the parameter is an \p
 ...
 */
        ...
        /*! \brief The class \p error_category serves as a base class for types used to
 * identify the source and encoding of a particular category of error code.
 * Classes may be derived from \p error_category to support categories of errors
 * in addition to those defined in the C++ International Standard.
 */
   ...
   */
        ...
        /*! \return A reference to an object of a type derived from class \p
 * error_category. \note The object's \p default_error_condition and \p
 * equivalent virtual functions shall behave as specified for the class \p
 * error_category. The object's \p name virtual function shall return a pointer
 * to the string <tt>"generic"</tt>.
 */
        inline const error_category& generic_category(void);

        /*! \return A reference to an object of a type derived from class \p
 * error_category. \note The object's \p equivalent virtual functions shall
 * behave as specified for class \p error_category. The object's \p name virtual
 * function shall return a pointer to the string <tt>"system"</tt>. The object's
 * \p default_error_condition virtual function shall behave as follows:
 *
 *        If the argument <tt>ev</tt> corresponds to a POSIX <tt>errno</tt>
 * value \c posv, the function shall return
 * <tt>error_condition(ev,generic_category())</tt>. Otherwise, the function
 * shall return <tt>error_condition(ev,system_category())</tt>. What constitutes
 ...
 */
        ...
        /*! \brief The class \p error_code describes an object used to hold error code
...
// XXX WAR msvc's problem with enable_if
                       ...
                       typename thrust::detail::enable_if<
                           is_error_code_enum<ErrorCodeEnum>::value>::type* = 0
...
// XXX WAR msvc's problem with enable_if
#if THRUST_HOST_COMPILER != THRUST_HOST_COMPILER_MSVC
            typename thrust::detail::enable_if<is_error_code_enum<ErrorCodeEnum>::value,
                                               error_code>::type&
#else
            error_code&
#endif // THRUST_HOST_COMPILER_MSVC
   ...
   */
            ...
            /*! \return An \p error_category describing the category of this \p error_code
   * object.
   */
 ...
 */
        ...
        /*! \return <tt>lhs.category() < rhs.category() || lhs.category() ==
 * rhs.category() && lhs.value() < rhs.value()</tt>.
 */
 ...
 */
        ...
        /*! \brief The class \p error_condition describes an object used to hold values
 * identifying error conditions.
 *
 *  \note \p error_condition values are portable abstractions, while \p
 * error_code values are implementation specific.
 */
        class error_condition
        {
        public:
   ...
   *        <tt>is_error_condition_enum<ErrorConditionEnum>::value</tt> is
   * <tt>true</tt>.
   */
...
// XXX WAR msvc's problem with enable_if
            ...
            /*! Assigns to this \p error_code object from an error value and an \p
   * error_category. \param val The new value to return from <tt>value()</tt>.
...
// XXX WAR msvc's problem with enable_if
#if THRUST_HOST_COMPILER != THRUST_HOST_COMPILER_MSVC
            typename thrust::detail::enable_if<is_error_condition_enum<ErrorConditionEnum>::value,
   ...
   */
            ...
            /*! \return A <tt>const</tt> reference to the \p error_category encoded by
   * this \p error_condition.
   */
            inline const error_category& category(void) const;
 ...
 */
        ...
        /*! \return <tt>lhs.category() == rhs.category() && lhs.value() ==
 * rhs.value()</tt>.
 */
        inline bool operator==(const error_code& lhs, const error_code& rhs);

        /*! \return <tt>lhs.category().equivalent(lhs.value(), rhs) ||
 * rhs.category().equivalent(lhs,rhs.value())</tt>.
 */
        inline bool operator==(const error_code& lhs, const error_condition& rhs);

        /*! \return <tt>rhs.category().equivalent(lhs.value(), lhs) ||
 * lhs.category().equivalent(rhs, lhs.value())</tt>.
 */
