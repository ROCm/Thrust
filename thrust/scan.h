 ...
 *                         and if \c x and \c y are objects of \c
 OutputIterator's
 *                         \c value_type, then <tt>x + y</tt> is defined. If \c
 T is
 *                         \c OutputIterator's \c value_type, then <tt>T(0)</tt>
 is
 *                         defined.
 *
 *  \pre \p first may equal \p result but the range <tt>[first, last)</tt> and
 the range <tt>[result, result + (last - first))</tt> shall not overlap
 otherwise.
 *
 *  The following code snippet demonstrates how to use \p inclusive_scan to
 compute an in-place
 ...
 * \c InputIterator's \c value_type is convertible to \c OutputIterator's \c
 * value_type. \tparam OutputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>
 *                         and \c OutputIterator's \c value_type is convertible
 * to both \c AssociativeOperator's \c first_argument_type and \c
 * second_argument_type. \tparam AssociativeOperator is a model of <a
 * href="http://www.sgi.com/tech/stl/BinaryFunction.html">Binary Function</a>
 *                              and \c AssociativeOperator's \c result_type is
 *                              convertible to \c OutputIterator's \c
 * value_type.
 *
 *  \pre \p first may equal \p result but the range <tt>[first, last)</tt> and
 * the range <tt>[result, result + (last - first))</tt> shall not overlap
 ...
 *
 *  The following code snippet demonstrates how to use \p inclusive_scan to
 * compute an in-place prefix sum using the \p thrust::host execution policy for
 * parallelization:
 *
 *  \code
 ...
 *
 *  thrust::inclusive_scan(thrust::host, data, data + 10, data, binary_op); //
 * in-place scan
 *
 ...
 * href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>,
 *                         and if \c x and \c y are objects of \c
 * OutputIterator's \c value_type, then <tt>x + y</tt> is defined. If \c T is \c
 * OutputIterator's \c value_type, then <tt>T(0)</tt> is defined.
 *
 *  \pre \p first may equal \p result but the range <tt>[first, last)</tt> and
 * the range <tt>[result, result + (last - first))</tt> shall not overlap
 * otherwise.
 *
 *  The following code snippet demonstrates how to use \p exclusive_scan to
 * compute an in-place prefix sum using the \p thrust::host execution policy for
 * parallelization:
 *
 *  \code
 ...
 * href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>,
 *                         and if \c x and \c y are objects of \c
 * OutputIterator's \c value_type, then <tt>x + y</tt> is defined. \tparam T is
 ...
 *  int data[6] = {1, 0, 2, 2, 1, 3};
 *
 *  thrust::exclusive_scan(thrust::host, data, data + 6, data, 4); // in-place
 * scan
 *
 ...
 *                         and \c OutputIterator's \c value_type is convertible
 * to both \c AssociativeOperator's \c first_argument_type and \c
 * second_argument_type. \tparam T is convertible to \c OutputIterator's \c
 * value_type. \tparam AssociativeOperator is a model of <a
 * href="http://www.sgi.com/tech/stl/BinaryFunction.html">Binary Function</a>
 *                              and \c AssociativeOperator's \c result_type is
 *                              convertible to \c OutputIterator's \c
 * value_type.
 ...
 *  thrust::maximum<int> binary_op;
 *
 *  thrust::exclusive_scan(data, data + 10, data, 1, binary_op); // in-place
 * scan
 *
 *  // data is now {1, 1, 1, 2, 2, 2, 4, 4, 4, 4 }
 *  \endcode
 ...
 */
    ...
    /*! \p inclusive_scan_by_key computes an inclusive key-value or 'segmented'
 * prefix sum operation. The term 'inclusive' means that each result includes
 ...
 *  predicate used to compare adjacent keys.  Specifically, consecutive
 * iterators <tt>i</tt> and <tt>i+1</tt> in the range <tt>[first1, last1)</tt>
 ...
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a> and
 * \c InputIterator2's \c value_type is convertible to \c OutputIterator's \c
 * value_type. \tparam OutputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>,
 *                         and if \c x and \c y are objects of \c
 * OutputIterator's \c value_type, then <tt>binary_op(x,y)</tt> is defined.
 *
 *  \pre \p first1 may equal \p result but the range <tt>[first1, last1)</tt>
 * and the range <tt>[result, result + (last1 - first1))</tt> shall not overlap
 * otherwise. \pre \p first2 may equal \p result but the range <tt>[first2,
 * first2 + (last1 - first1)</tt> and range <tt>[result, result + (last1 -
 * first1))</tt> shall not overlap otherwise.
 *
 ...
 *
 *  thrust::inclusive_scan_by_key(thrust::host, keys, keys + 10, vals, vals); //
 * in-place scan
 *
 ...
 *  belong to the same segment if <tt>binary_pred(*i, *(i+1))</tt> is true, and
 * belong to different segments otherwise.
 *
 *  This version of \p inclusive_scan_by_key assumes \c plus as the associative
 *  operator used to perform the prefix sum. When the input and output sequences
 *  are the same, the scan is performed in-place.
 ...
 *  thrust::equal_to<int> binary_pred;
 *
 *  thrust::inclusive_scan_by_key(keys, keys + 10, vals, vals, binary_pred); //
 * in-place scan
 *
 *  // data is now {1, 2, 3, 1, 2, 1, 1, 2, 3, 4};
 *  \endcode
 ...
 *
 *  thrust::inclusive_scan_by_key(thrust::host, keys, keys + 10, vals, vals,
 * binary_pred, binary_op); // in-place scan
 *
 *  // data is now {1, 2, 3, 1, 2, 1, 1, 2, 3, 4};
 *  \endcode
 *
 ...
 *  different segments otherwise.
 *
 *  Refer to the most general form of \p exclusive_scan_by_key for additional
 * details.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 ...
 */
    ...
    /*! \p exclusive_scan_by_key computes an exclusive key-value or 'segmented'
 * prefix sum operation. The term 'exclusive' means that each result does not
 * include the corresponding input operand in the partial sum. The term
 * 'segmented' means that the partial sums are broken into distinct segments. In
 * other words, within each segment a separate exclusive scan operation is
 ...
 *  int init = 5;
 *
 *  thrust::exclusive_scan_by_key(key, key + 10, vals, vals, init); // in-place
 * scan
 *
 *  // vals is now {5, 6, 7, 5, 6, 5, 5, 6, 7, 8};
 *  \endcode
 ...
 *  initialize the exclusive scan operation.
 *
 *  This version of \p exclusive_scan_by_key uses the binary predicate \c
 * binary_pred to compare adjacent keys.  Specifically, consecutive iterators
 * <tt>i</tt> and <tt>i+1</tt> in the range <tt>[first1, last1)</tt> belong to
 * the same segment if <tt>binary_pred(*i, *(i+1))</tt> is true, and belong to
 * different segments otherwise.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 ...
 *  thrust::equal_to<int> binary_pred;
 *
 *  thrust::exclusive_scan_by_key(thrust::host, key, key + 10, vals, vals, init,
 * binary_pred); // in-place scan
 *
 *  // vals is now {5, 6, 7, 5, 6, 5, 5, 6, 7, 8};
 *  \endcode
