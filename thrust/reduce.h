 ...
 * use \p inclusive_scan (which does not require commutativity) and select the
 ...
 * if \c x and \c y are objects of \p InputIterator's \c value_type, then <tt>x
 * + y</tt> is defined and is convertible to \p InputIterator's \c value_type.
 * If \c T is \c InputIterator's \c value_type, then <tt>T(0)</tt> is defined.
 *
 ...
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a> and
 * if \c x and \c y are objects of \p InputIterator's \c value_type, then <tt>x
 * + y</tt> is defined and is convertible to \p T. \tparam T is convertible to
 * \p InputIterator's \c value_type.
 *
 *  The following code snippet demonstrates how to use \p reduce to compute
 *  the sum of a sequence of integers including an intialization value using the
 ...
 * reduce requires associativity of \p binary_op to parallelize the reduction.
 *
 *  Note that \p reduce also assumes that the binary reduction operator (in this
 ...
 *  \tparam InputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a> and
 * \c InputIterator's \c value_type is convertible to \c T. \tparam T is a model
 * of <a href="http://www.sgi.com/tech/stl/Assignable.html">Assignable</a>, and
 * is convertible to \p BinaryFunction's \c first_argument_type and \c
 ...
 * keys_last)</tt> that are equal, \p reduce_by_key copies the first element of
 * the group to the \c keys_output. The corresponding values in the range are
 * reduced using the \c plus and the result copied to \c values_output.
 *
 ...
 * keys_last)</tt> that are equal, \p reduce_by_key copies the first element of
 * the group to the \c keys_output. The corresponding values in the range are
 * reduced using the \c BinaryFunction \c binary_op and the result copied to \c
 * values_output. Specifically, if consecutive key iterators \c i and \c (i + 1)
 * are such that <tt>binary_pred(*i, *(i+1))</tt> is \c true, then the
 ...
 * href="http://www.sgi.com/tech/stl/BinaryFunction.html">Binary Function</a>
 *          and \c BinaryFunction's \c result_type is convertible to \c
 * OutputIterator2's \c value_type.
 *
 *  \pre The input ranges shall not overlap either output range.
 *
