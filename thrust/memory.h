...
/*! \p pointer stores a pointer to an object allocated in memory. Like \p
 * device_ptr, this type ensures type safety when dispatching standard
 * algorithms on ranges resident in memory.
 *
 *  \p pointer generalizes \p device_ptr by relaxing the backend system
 * associated with the \p pointer. Instead of the backend system specified by \p
 * THRUST_DEFAULT_DEVICE_BACKEND, \p pointer's system is given by its second
 * template parameter, \p Tag. For the purpose of Thrust dispatch,
 *  <tt>device_ptr<Element></tt> and <tt>pointer<Element,device_system_tag></tt>
 * are considered equivalent.
 *
 *  The raw pointer encapsulated by a \p pointer may be obtained through its
 * <tt>get</tt> member function or the \p raw_pointer_cast free function.
 *
 *  \tparam Element specifies the type of the pointed-to object.
 *
 *  \tparam Tag specifies the system with which this \p pointer is associated.
 * This may be any Thrust backend system, or a user-defined tag.
 *
 *  \tparam Reference allows the client to specify the reference type returned
 * upon derereference. By default, this type is
 * <tt>reference<Element,pointer></tt>.
 *
 *  \tparam Derived allows the client to specify the name of the derived type
 * when \p pointer is used as a base class. This is useful to ensure that
 * arithmetic on values of the derived type return values of the derived type as
 * a result. By default, this type is <tt>pointer<Element,Tag,Reference></tt>.
 *
 *  \note \p pointer is not a smart pointer; it is the client's responsibility
 * to deallocate memory pointer to by \p pointer.
 *
 *  \see device_ptr
 *  \see reference
...
/*! \p reference is a wrapped reference to an object stored in memory. \p
 * reference generalizes \p device_reference by relaxing the type of pointer
 * associated with the object. \p reference is the type of the result of
 * dereferencing a tagged pointer-like object such as \p pointer, and
 *  intermediates operations on objects existing in a remote memory.
 *
 *  \tparam Element specifies the type of the referent object.
 *  \tparam Pointer specifies the type of the result of taking the address of \p
 * reference. \tparam Derived allows the client to specify the name of the
 * derived type when \p reference is used as a base class. This is useful to
 * ensure that assignment to objects of the derived type return values of the
 * derived type as a result. By default, this type is
 * <tt>reference<Element,Pointer></tt>.
 */
// define pointer for the purpose of Doxygenating it
// it is actually defined elsewhere
 ...
 */
    ...
    /*! This version of \p malloc allocates untyped uninitialized storage associated
 * with a given system.
 *
 *  \param system The Thrust system with which to associate the storage.
 *  \param n The number of bytes of storage to allocate.
 *  \return If allocation succeeds, a pointer to the allocated storage; a null
 * pointer otherwise. The pointer must be deallocated with \p thrust::free.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p DerivedPolicy must be publically derived from
 * <code>thrust::execution_policy<DerivedPolicy></code>.
 *
 *  The following code snippet demonstrates how to use \p malloc to allocate a
 * range of memory associated with Thrust's device system.
 *
 *  \code
 ...
 *  thrust::pointer<void,thrust::device_space_tag> void_ptr =
 ...
 */
    ...
    /*! This version of \p malloc allocates typed uninitialized storage associated
 * with a given system.
 *
 *  \param system The Thrust system with which to associate the storage.
 *  \param n The number of elements of type \c T which the storage should
 * accomodate. \return If allocation succeeds, a pointer to an allocation large
 * enough to accomodate \c n elements of type \c T; a null pointer otherwise.
 ...
 *
 *  The following code snippet demonstrates how to use \p malloc to allocate a
 * range of memory to accomodate integers associated with Thrust's device
 * system.
 *
 *  \code
 *  #include <thrust/memory.h>
 ...
 */
    ...
    /*! \p get_temporary_buffer returns a pointer to storage associated with a given
 * Thrust system sufficient to store up to \p n objects of type \c T. If not
 * enough storage is available to accomodate \p n objects, an implementation may
 * return a smaller buffer. The number of objects the returned buffer can
 * accomodate is also returned.
 *
 *  Thrust uses \p get_temporary_buffer internally when allocating temporary
 * storage required by algorithm implementations.
 *
 *  The storage allocated with \p get_temporary_buffer must be returned to the
 * system with \p return_temporary_buffer.
 *
 *  \param system The Thrust system with which to associate the storage.
 *  \param n The requested number of objects of type \c T the storage should
 * accomodate. \return A pair \c p such that <tt>p.first</tt> is a pointer to
 * the allocated storage and <tt>p.second</tt> is the number of contiguous
 * objects of type \c T that the storage can accomodate. If no storage can be
 * allocated, <tt>p.first</tt> if no storage can be obtained. The storage must
 * be returned to the system using \p return_temporary_buffer.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p DerivedPolicy must be publically derived from
 * <code>thrust::execution_policy<DerivedPolicy></code>.
 *
 *  The following code snippet demonstrates how to use \p get_temporary_buffer
 * to allocate a range of memory to accomodate integers associated with Thrust's
 * device system.
 *
 *  \code
 *  #include <thrust/memory.h>
 ...
 *
 *  thrust::device_system_tag device_sys;
 *  ptr_and_size_t ptr_and_size = thrust::get_temporary_buffer<int>(device_sys,
 * N);
 *
 ...
 */
    template <typename T, typename DerivedPolicy>
    __host__ __device__ thrust::pair<thrust::pointer<T, DerivedPolicy>,
 ...
 * null, \p free does nothing.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p ptr shall have been returned by a previous call to
 * <tt>thrust::malloc(system, n)</tt> or <tt>thrust::malloc<T>(system, n)</tt>
 * for some type \c T.
 *
 *  The following code snippet demonstrates how to use \p free to deallocate a
 * range of memory previously allocated with \p thrust::malloc.
 *
 *  \code
 *  #include <thrust/memory.h>
 *  ...
 ...
 */
    ...
    __host__ __device__ void
             free(const thrust::detail::execution_policy_base<DerivedPolicy>& system, Pointer ptr);

    /*! \p return_temporary_buffer deallocates storage associated with a given
 * Thrust system previously allocated by \p get_temporary_buffer.
 *
 *  Thrust uses \p return_temporary_buffer internally when deallocating
 * temporary storage required by algorithm implementations.
 *
 *  \param system The Thrust system with which the storage is associated.
 *  \param p A pointer previously returned by \p thrust::get_temporary_buffer.
 * If \p ptr is null, \p return_temporary_buffer does nothing.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *
 *  \pre \p p shall have been previously allocated by \p
 * thrust::get_temporary_buffer.
 *
 *  The following code snippet demonstrates how to use \p
 * return_temporary_buffer to deallocate a range of memory previously allocated
 * by \p get_temporary_buffer.
 *
 *  \code
 *  #include <thrust/memory.h>
 *  ...
 ...
 *  \return <tt>ptr.get()</tt>, if the expression is well formed; <tt>ptr</tt>,
 * otherwise. \see raw_reference_cast
 */
    template <typename Pointer>
    __host__ __device__ inline typename thrust::detail::pointer_traits<Pointer>::raw_pointer
             raw_pointer_cast(const Pointer& ptr);

    /*! \p raw_reference_cast creates a "raw" reference from a wrapped reference
 * type, simply returning the underlying reference, should it exist.
 *
 *  If the argument is not a reference wrapper, the result is a reference to the
 * argument.
 *
 *  \param ref The reference of interest.
 *  \return <tt>*thrust::raw_pointer_cast(&ref)</tt>.
 *  \note There are two versions of \p raw_reference_cast. One for
 * <tt>const</tt> references, and one for non-<tt>const</tt>. \see
 * raw_pointer_cast
 */
    template <typename T>
    __host__ __device__ inline typename detail::raw_reference<T>::type raw_reference_cast(T& ref);
 ...
 */
    template <typename T>
    __host__ __device__ inline typename detail::raw_reference<const T>::type
