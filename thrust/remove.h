 ...
 */
    ...
    /*! \p remove removes from the range <tt>[first, last)</tt> all elements that
 * are equal to \p value. That is, \p remove returns an iterator \p new_last
 * such that the range <tt>[first, new_last)</tt> contains no elements equal to
 *  \p value. The iterators in the range <tt>[new_first,last)</tt> are all still
 *  dereferenceable, but the elements that they point to are unspecified. \p
 * remove is stable, meaning that the relative order of elements that are not
 * equal to \p value is unchanged.
 *
 ...
 * Comparable</a>, and objects of type \p T can be compared for equality with
 * objects of \p ForwardIterator's \c value_type.
 *
 *  The following code snippet demonstrates how to use \p remove to remove a
 * number of interest from a range using the \p thrust::host execution policy
 * for parallelization:
 *
 *  \code
 ...
 *  \endcode
 *
 *  \note The meaning of "removal" is somewhat subtle. \p remove does not
 ...
 * example, if \c V is a device_vector, <tt>remove(V.begin(), V.end(), 0)</tt>
 * does not change <tt>V.size()</tt>: \c V will contain just as many elements as
 * it did before. \p remove returns an iterator that points to the end of the
 * resulting range after elements have been removed from it; it follows that the
 * elements after that iterator are of no interest, and may be discarded. If you
 * are removing elements from a <a
 * href="http://www.sgi.com/tech/stl/Sequence.html">Sequence</a>, you may simply
 * erase them. That is, a reasonable way of removing elements from a <a
 * href="http://www.sgi.com/tech/stl/Sequence.html">Sequence</a> is
 ...
 *
 *  The following code snippet demonstrates how to use \p remove to remove a
 * number of interest from a range.
 *
 *  \code
 *  #include <thrust/remove.h>
 *  ...
 ...
 *  <tt>[first, last)</tt> to a range beginning at \p result. The return value
 * is the end of the resulting range. This operation is stable, meaning that the
 ...
 *  \param first The beginning of the range of interest.
 *  \param last The end of the range of interest.
 *  \param result The resulting range is copied to the sequence beginning at
 * this location. \param value The value to omit from the copied range. \return
 * An OutputIterator pointing to the end of the resulting range of elements
 ...
 * \p InputIterator's \c value_type is convertible to a type in \p
 * OutputIterator's set of \c value_types. \tparam OutputIterator is a model of
 * <a href="http://www.sgi.com/tech/stl/OutputIterator.html">Output
 * Iterator</a>. \tparam T is a model of <a
 * href="http://www.sgi.com/tech/stl/EqualityComparable">Equality
 * Comparable</a>, and objects of type \p T can be compared for equality with
 * objects of \p InputIterator's \c value_type.
 *
 *  \pre The range <tt>[first, last)</tt> shall not overlap the range
 ...
 */
    ...
    /*! \p remove_if removes from the range <tt>[first, last)</tt> every element \p
 * x such that <tt>pred(x)</tt> is \c true. That is, \p remove_if returns an
 ...
 *  <tt>[new_last,last)</tt> are all still dereferenceable, but the elements
 * that they point to are unspecified. \p remove_if is stable, meaning that the
 ...
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *  \tparam ForwardIterator is a model of <a
 * href="http://www.sgi.com/tech/ForwardIterator.html">Forward Iterator</a>, \p
 * ForwardIterator is mutable, and \p ForwardIterator's \c value_type is
 * convertible to \p Predicate's \c argument_type. \tparam Predicate is a model
 * of <a href="http://www.sgi.com/tech/Predicate.html">Predicate</a>.
 *
 *  The following code snippet demonstrates how to use \p remove_if to remove
 *  all even numbers from an array of integers using the \p thrust::host
 ...
 *  This operation is stable, meaning that the relative order of the elements
 * that are copied is the same as the range <tt>[first,last)</tt>.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 *  \param exec The execution policy to use for parallelization.
 ...
 *  \param last The end of the range of interest.
 *  \param result The resulting range is copied to the sequence beginning at
 * this location. \param pred A predicate to evaluate for each element of the
 * range <tt>[first,last)</tt>. Elements for which \p pred evaluates to \c false
 * are not copied to the resulting sequence. \return An OutputIterator pointing
 * to the end of the resulting range.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *  \tparam InputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>, \p
 * InputIterator's \c value_type is convertible to a type in \p OutputIterator's
 * set of \c value_types, and \p InputIterator's \c value_type is convertible to
 * \p Predicate's \c argument_type. \tparam OutputIterator is a model of <a
 ...
 *  a sequence of numbers to an output range while omitting even numbers using
 ...
 * dereferenceable, but the elements that they point to are unspecified. \p
 * remove_if is stable, meaning that the relative order of elements that are not
 ...
 *  \param pred A predicate to evaluate for each element of the range
 *              <tt>[stencil, stencil + (last - first))</tt>. Elements for which
 * \p pred evaluates to \c true are removed from the sequence <tt>[first,
 * last)</tt> \return A ForwardIterator pointing to the end of the resulting
 * range of elements for which \p pred evaluated to \c true.
 *
 *  \tparam DerivedPolicy The name of the derived execution policy.
 *  \tparam ForwardIterator is a model of <a
 * href="http://www.sgi.com/tech/ForwardIterator.html">Forward Iterator</a> and
 * \p ForwardIterator is mutable. \tparam InputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>, and
 * \p InputIterator's \c value_type is convertible to \p Predicate's \c
 * argument_type. \tparam Predicate is a model of <a
 * href="http://www.sgi.com/tech/Predicate.html">Predicate</a>.
 *
 *  \pre The range <tt>[first, last)</tt> shall not overlap the range
 * <tt>[result, result + (last - first))</tt>. \pre The range <tt>[stencil,
 * stencil + (last - first))</tt> shall not overlap the range <tt>[result,
 * result + (last - first))</tt>.
 *
 *  The following code snippet demonstrates how to use \p remove_if to remove
 *  specific elements from an array of integers using the \p thrust::host
 * execution policy for parallelization:
 *
 *  \code
 *  #include <thrust/remove.h>
 ...
 *  int S[N] = {0, 1, 1, 1, 0, 0};
 *
 *  int *new_end = thrust::remove_if(thrust::host, A, A + N, S,
 * thrust::identity<int>());
 *  // The first three values of A are now {1, 5, 7}
 *  // Values beyond new_end are unspecified
 *  \endcode
 *
 *  \note The range <tt>[first, last)</tt> is not permitted to overlap with the
 * range <tt>[stencil, stencil + (last - first))</tt>.
 *
 *  \see http://www.sgi.com/tech/stl/remove_if.html
 *  \see remove
 *  \see remove_copy
 ...
 * href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>, \p
 * InputIterator1's \c value_type is convertible to a type in \p
 * OutputIterator's set of \c value_types. \tparam InputIterator2 is a model of
 * <a href="http://www.sgi.com/tech/stl/InputIterator.html">Input Iterator</a>,
 *          and \p InputIterator2's \c value_type is convertible to \p
 * Predicate's \c argument_type. \tparam OutputIterator is a model of <a
 * href="http://www.sgi.com/tech/stl/OutputIterator.html">Output Iterator</a>.
 *  \tparam Predicate is a model of <a
 * href="http://www.sgi.com/tech/stl/Predicate.html">Predicate</a>.
 *
 *  \pre The range <tt>[stencil, stencil + (last - first))</tt> shall not
 * overlap the range <tt>[result, result + (last - first))</tt>.
 *
 *  The following code snippet demonstrates how to use \p remove_copy_if to copy
 ...
 *  int S[N] = { 1, 1,  0, 1, 0, 1};
 *  int result[2];
 *  thrust::remove_copy_if(thrust::host, V, V + N, S, result,
 * thrust::identity<int>());
 *  // V remains {-2, 0, -1, 0, 1, 2}
 *  // result is now {-1, 1}
 *  \endcode
