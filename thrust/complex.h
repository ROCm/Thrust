 ...
 * functionally equivalent to it, but can also be used in device code which
 ...
 */
   ...
   */
        template <typename X>
        inline __host__ complex(const std::complex<X>& z);
 ...
 * complex should be compatible and the type of the returned \p complex is the
 * promoted type of the two arguments.
 *
 *  \param x The base.
 *  \param y The exponent.
 */
    template <typename T, typename U>
    __host__ __device__ complex<typename detail::promoted_numerical_type<T, U>::type>
                        pow(const complex<T>& x, const complex<U>& y);

    /*! Returns a \p complex number raised to a scalar. The type of the \p complex
 * should be compatible with the scalar and the type of the returned \p complex
