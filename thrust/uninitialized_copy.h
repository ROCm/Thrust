 ...
 * operations. If each iterator in the range <tt>[result, result + (last -
 * first))</tt> points to uninitialized memory, then \p uninitialized_copy
 * creates a copy of <tt>[first, last)</tt> in that range. That is, for each
 * iterator \c i in the input, \p uninitialized_copy creates a copy of \c *i in
 * the location pointed to by the corresponding iterator in the output range by
 * \p ForwardIterator's \c value_type's copy constructor with *i as its
 * argument.
 *
 ...
 *          \p ForwardIterator is mutable, and \p ForwardIterator's \c
 * value_type has a constructor that takes a single argument whose type is \p
 ...
 * otherwise.
 *
 *  The following code snippet demonstrates how to use \p uninitialized_copy to
 ...
 *  thrust::uninitialized_copy(thrust::device, input.begin(), input.end(),
 * array);
 *
 *  // Int x = array[i];
 ...
 *  an object and then creates an object at that location by calling a
 * constructor. Occasionally, however, it is useful to separate those two
 * operations. If each iterator in the range <tt>[result, result + n)</tt>
 * points to uninitialized memory, then \p uninitialized_copy_n creates a copy
 * of <tt>[first, first + n)</tt> in that range. That is, for each iterator \c i
 * in the input, \p uninitialized_copy_n creates a copy of \c *i in the location
 * pointed to by the corresponding iterator in the output range by \p
 * InputIterator's \c value_type's copy constructor with *i as its argument.
 *
 *  The algorithm's execution is parallelized as determined by \p exec.
 *
 *  \param exec The execution policy to use for parallelization.
 ...
 * InputIterator's \c value_type.
 *
 *  \pre \p first may equal \p result, but the range <tt>[first, first + n)</tt>
 * and the range <tt>[result, result + n)</tt> shall not overlap otherwise.
 *
 *  The following code snippet demonstrates how to use \p uninitialized_copy to
 * initialize a range of uninitialized memory using the \p thrust::device
 * execution policy for parallelization:
