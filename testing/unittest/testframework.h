...
typedef unittest::type_list<signed char, signed short, signed int, signed long, signed long long>
    SignedIntegralTypes;
...
typedef unittest::type_list<char, signed char, unsigned char, short, unsigned short>
    SmallIntegralTypes;
...
inline std::string base_class_name(const std::string& name)
{
    ...
    return result.replace(result.find_first_of("<"), result.size(), "");
}
