...
#define ASSERT_THROWS(X, Y)                                                      \
    {                                                                            \
        ...
        {                                                                        \
            ...
            f << "[" << __FILE__ << ":" << __LINE__ << "] did not throw " << #Y; \
            throw f;                                                             \
        }                                                                        \
    }
...
namespace unittest
{
    ...
    struct value_type
    {
        typedef typename thrust::detail::remove_const<
            typename thrust::detail::remove_reference<T>::type>::type type;
    };
    ...
    template <typename T1, typename T2>
    void assert_equal_quiet(const T1&          a,
                            const T2&          b,
                            const std::string& filename = "unknown",
                            int                lineno   = -1)
    {
    ...
    // define our own abs() because std::abs() isn't portable for all types for some
    // reason
    template <typename T>
    T abs(const T& x)
    {
    ...
    inline bool
        almost_equal(const double& a, const double& b, const double& a_tol, const double& r_tol)
    {
    ...
    template <typename T1, typename T2>
    void assert_almost_equal(const T1&          a,
                             const T2&          b,
                             const std::string& filename = "unknown",
                             int                lineno   = -1,
    ...
    template <typename T1, typename T2>
    void assert_almost_equal(const thrust::complex<T1>& a,
                             const thrust::complex<T2>& b,
                             const std::string&         filename = "unknown",
                             int                        lineno   = -1,
                             double                     a_tol    = DEFAULT_ABSOLUTE_TOL,
                             double                     r_tol    = DEFAULT_RELATIVE_TOL)
    ...
    void assert_equal(ForwardIterator1   first1,
                      ...
                      int                lineno   = -1)
    {
        ...
        {
            f << "Sequences agree through " << min_length << " positions [type='"
              << type_name<InputType>() << "']\n";
        }
    ...
    template <typename ForwardIterator1, typename ForwardIterator2>
    void assert_almost_equal(ForwardIterator1   first1,
                             ForwardIterator1   last1,
                             ForwardIterator2   first2,
                             ForwardIterator2   last2,
                             const std::string& filename = "unknown",
                             ...
                             const double       r_tol    = DEFAULT_RELATIVE_TOL)
    {
        ...
        assert_equal(first1,
                     ...
                     first2,
                     last2,
                     almost_equal_to<InputType>(a_tol, r_tol),
                     filename,
                     lineno);
    }

    template <typename T, typename Alloc>
    void assert_equal(const thrust::host_vector<T, Alloc>& A,
                      const thrust::host_vector<T, Alloc>& B,
                      const std::string&                   filename = "unknown",
                      int                                  lineno   = -1)
    {
    ...
    template <typename T, typename Alloc>
    void assert_almost_equal(const thrust::host_vector<T, Alloc>& A,
                             const thrust::host_vector<T, Alloc>& B,
                             const std::string&                   filename = "unknown",
                             int                                  lineno   = -1,
                             const double                         a_tol    = DEFAULT_ABSOLUTE_TOL,
    ...
    void assert_equal(const thrust::host_vector<T, Alloc1>&   A,
                      const thrust::device_vector<T, Alloc2>& B,
                      const std::string&                      filename = "unknown",
                      int                                     lineno   = -1)
    {
    ...
    template <typename T, typename Alloc1, typename Alloc2>
    void assert_equal(const thrust::device_vector<T, Alloc1>& A,
                      const thrust::host_vector<T, Alloc2>&   B,
                      const std::string&                      filename = "unknown",
                      int                                     lineno   = -1)
    {
    ...
    template <typename T, typename Alloc>
    void assert_equal(const thrust::device_vector<T, Alloc>& A,
                      const thrust::device_vector<T, Alloc>& B,
                      const std::string&                     filename = "unknown",
                      int                                    lineno   = -1)
    {
    ...
    template <typename T, typename Alloc1, typename Alloc2>
    void assert_almost_equal(const thrust::host_vector<T, Alloc1>&   A,
                             const thrust::device_vector<T, Alloc2>& B,
                             const std::string&                      filename = "unknown",
                             int                                     lineno   = -1,
                             const double                            a_tol = DEFAULT_ABSOLUTE_TOL,
    ...
    template <typename T, typename Alloc1, typename Alloc2>
    void assert_almost_equal(const thrust::device_vector<T, Alloc1>& A,
                             const thrust::host_vector<T, Alloc2>&   B,
                             const std::string&                      filename = "unknown",
                             int                                     lineno   = -1,
                             const double                            a_tol = DEFAULT_ABSOLUTE_TOL,
    ...
    template <typename T, typename Alloc>
    void assert_almost_equal(const thrust::device_vector<T, Alloc>& A,
                             const thrust::device_vector<T, Alloc>& B,
                             const std::string&                     filename = "unknown",
                             int                                    lineno   = -1,
                             const double                           a_tol    = DEFAULT_ABSOLUTE_TOL,
