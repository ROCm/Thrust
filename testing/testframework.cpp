...
// #include backends' testframework.h, if they exist and are required for the
// build
...
const size_t standard_test_sizes[] = {0,
                                      1,
                                      2,
                                      3,
                                      4,
...
void usage(int argc, char** argv)
{
    ...
    std::cout << indent << indent << "--sizes=default tests sizes up to " << default_threshold
              << "\n";
    std::cout << indent << indent << "--sizes=large   tests sizes up to " << large_threshold
              << " (0.25 GB memory)\n";
    std::cout << indent << indent << "--sizes=huge    tests sizes up to " << huge_threshold
              << " (1.50 GB memory)\n";
    std::cout << indent << indent << "--sizes=epic    tests sizes up to " << epic_threshold
              << " (3.00 GB memory)\n";
...
struct TestResult
{
    ...
    TestResult(const TestStatus   status,
               /*std::*/ clock_t  elapsed,
               const UnitTest&    u,
...
bool UnitTestDriver::run_tests(std::vector<UnitTest*>& tests_to_run, const ArgumentMap& kwargs)
{
    ...
    for(size_t i = 0; i < tests_to_run.size(); i++)
    {
        ...
        {
            record_result(
                TestResult(Failure, std::numeric_limits</*std::*/ clock_t>::max(), test, f.message),
                test_results);
        }
        catch(unittest::UnitTestKnownFailure& f)
        {
            record_result(
                TestResult(
                    KnownFailure, std::numeric_limits</*std::*/ clock_t>::max(), test, f.message),
                test_results);
        }
        catch(std::bad_alloc& e)
        {
            record_result(
                TestResult(Error, std::numeric_limits</*std::*/ clock_t>::max(), test, e.what()),
                test_results);
        }
        catch(unittest::UnitTestError& e)
        {
            record_result(
                TestResult(Error, std::numeric_limits</*std::*/ clock_t>::max(), test, e.message),
                test_results);
        }
            ...
            if(verbose)
            {
                ...
                case Pass:
                    ...
                    std::cout << std::setw(10)
                              << 1000.f * float(test_results.back().elapsed) / CLOCKS_PER_SEC
                              << " ms";
                    break;
                case Failure:
                    std::cout << "\r[FAILURE]           ";
...
bool UnitTestDriver::run_tests(const ArgumentSet& args, const ArgumentMap& kwargs)
{
    ...
    {
        // all non-keyword arguments are assumed to be test names or partial test
        // names
