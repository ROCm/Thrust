...
#define RECORD_RESULT(name, value, units)                                                      \
    {                                                                                          \
        std::cout << "  <result  name=\"" << name << "\"  value=\"" << value << "\"  units=\"" \
                  << units << "\"/>" << std::endl;                                             \
    }
...
#define RECORD_VARIABLE(name, value)                                                     \
    {                                                                                    \
        std::cout << "  <variable  name=\"" << name << "\"  value=\"" << value << "\"/>" \
                  << std::endl;                                                          \
    }
#define RECORD_TEST_STATUS(result, message)                                                    \
    {                                                                                          \
        std::cout << "  <status  result=\"" << result << "\"  message=\"" << message << "\"/>" \
                  << std::endl;                                                                \
    }
...
inline void RECORD_PLATFORM_INFO(void)
{
    ...
    std::cout << "    <property name=\"revision\""
              << " "
              << "value=\"" << deviceProp.major << "." << deviceProp.minor << "\"/>" << std::endl;
    std::cout << "    <property name=\"global memory\""
              << " "
              << "value=\"" << deviceProp.totalGlobalMem << "\"  units=\"bytes\"/>" << std::endl;
    std::cout << "    <property name=\"multiprocessors\""
              << " "
              << "value=\"" << deviceProp.multiProcessorCount << "\"/>" << std::endl;
    std::cout << "    <property name=\"cores\""
              << " "
              << "value=\"" << 8 * deviceProp.multiProcessorCount << "\"/>" << std::endl;
    std::cout << "    <property name=\"constant memory\""
              << " "
              << "value=\"" << deviceProp.totalConstMem << "\"  units=\"bytes\"/>" << std::endl;
    std::cout << "    <property name=\"shared memory per block\""
              << " "
              << "value=\"" << deviceProp.sharedMemPerBlock << "\"  units=\"bytes\"/>" << std::endl;
    std::cout << "    <property name=\"warp size\""
              << " "
              << "value=\"" << deviceProp.warpSize << "\"/>" << std::endl;
    std::cout << "    <property name=\"max threads per block\""
              << " "
              << "value=\"" << deviceProp.maxThreadsPerBlock << "\"/>" << std::endl;
    std::cout << "    <property name=\"clock rate\""
              << " "
              << "value=\"" << (deviceProp.clockRate * 1e-6f) << "\"  units=\"GHz\"/>" << std::endl;
    std::cout << "  </device>" << std::endl;
    std::cout << "  <compilation>" << std::endl;
#ifdef __HIP_PLATFORM_NVCC__
    std::cout << "    <property name=\"CUDA_VERSION\" value=\"" << CUDA_VERSION << "\"/>"
              << std::endl;
#endif
    std::cout << "    <property name=\"host compiler\" value=\"" << __HOST_COMPILER_NAME__ << " "
              << __HOST_COMPILER_VERSION__ << "\"/>" << std::endl;
